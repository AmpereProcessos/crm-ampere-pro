...
export const UtilsIdentifierSchema = z.enum(
  ["EQUIPMENT", "CREDITOR", "ACQUISITION_CHANNEL", "OPPORTUNITY_LOSS_REASON"],
  {
...
export const OpportunityLossReasonsSchema = z.object({
  identificador: z.literal("OPPORTUNITY_LOSS_REASON"),
  ativo: z.boolean({
    required_error: "Status de ativação da razão de perda não informado.",
    invalid_type_error: "Tipo não válido para status de ativação da razão de perda.",
  }),
  titulo: z.string({
    required_error: "Título da razão de perda não informado.",
    invalid_type_error: "Tipo não válido para título da razão de perda.",
  }),
  descricao: z.string({
    required_error: "Descrição da razão de perda não informada.",
    invalid_type_error: "Tipo não válido para descrição da razão de perda.",
  }),
});
export type TOpportunityLossReason = z.infer<typeof OpportunityLossReasonsSchema>;
export type TOpportunityLossReasonDTO = TOpportunityLossReason & { _id: string };
...
export type TUtil = TEquipment | TCreditor | TAcquisitionChannel | TOpportunityLossReason;
