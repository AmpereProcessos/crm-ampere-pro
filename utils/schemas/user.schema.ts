	...
	oportunidades: z.object({
		...
		excluir: z
			.boolean({
				...
				invalid_type_error: "Tipo não válido para permissão de exclusão de oportunidades.",
			})
			.optional()
			.nullable(),
	}),
...
export const GeneralUserSchema = z.object({
	...
	dataNascimento: z
		.string({
			...
			invalid_type_error: "Tipo não válido para a data de nascimento do usuário.",
		})
		.datetime({
...
export type TSessionUser = Pick<
	TUser,
	"administrador" | "nome" | "telefone" | "email" | "nome" | "avatar_url" | "idParceiro" | "idGrupo" | "permissoes" | "codigoIndicacaoConecta"
> & {
	id: string;
	parceiro: {
		nome: TPartner["nome"];
