...
const EquipmentSchema = z.object({
	id: z
		.string({ invalid_type_error: "Tipo não válido para ID do módulo." })
		.optional()
		.nullable(),
	categoria: z.union([
		z.literal("MÓDULO"),
	...
	modelo: z.string({
		required_error: "Modelo do módulo não informado.",
		invalid_type_error: "Tipo não válido para o modelo do módulo.",
	}),
	qtde: z.number({
		required_error: "Quantidade do módulo não informada.",
		invalid_type_error: "Tipo não válido para a quantidade do módulo.",
	}),
	potencia: z
		.number({
...
const TechnicalAnalysisPendencyCategoriesSchema = z.enum(
	[
		"PENDÊNCIA COMERCIAL",
		"PENDÊNCIA TERCEIROS",
		"PENDÊNCIA CONCESSIONÁRIA",
		"OUTROS",
	],
	{
...
export type TTechnicalAnalysisPendencyCategory = z.infer<
	typeof TechnicalAnalysisPendencyCategoriesSchema
>;
export const GeneralTechnicalAnalysisSchema = z.object({
	...
	nome: z.string({
		required_error: "Nome da análise técnica não informado.",
		invalid_type_error: "Tipo não válido para o nome da análise técnica.",
	}),
	idAnaliseReferencia: z
		.string({
			invalid_type_error: "Tipo não válido para o ID da análise de referência.",
		})
		.optional()
		.nullable(),
	status: z.string({
	...
	complexidade: z
		.union(
			[z.literal("SIMPLES"), z.literal("INTERMEDIÁRIO"), z.literal("COMPLEXO")],
			{
				required_error: "Complexidade da análise técnica não informada.",
				invalid_type_error:
					"Tipo não válido para a complexidade da análise técnica.",
			},
		)
		.optional()
		.nullable(),
	arquivosAuxiliares: z
		.string({
			invalid_type_error: "Tipo não válido para o link de arquivos auxiliares.",
		})
		.optional()
		.nullable(), // link de fotos do drone, por exemplo
	pendencias: z.array(
		z.object({
			...
			responsavel: z.object({
				...
				nome: z
					.string({
						required_error: "Nome do responsável não fornecido.",
				...
				avatar_url: z
					.string({
						invalid_type_error: "Avatar do responsável não fornecido.",
					})
					.optional()
					.nullable(),
			}),
			finalizado: z.boolean({
				...
				invalid_type_error:
					"Tipo não válido para o status de finalização da pendência não informada.",
			}),
			dataFinalizacao: z
				.string({
					invalid_type_error:
						"Tipo inválido para data de finalização da pendência.",
				})
				.optional()
				.nullable(),
			dataInsercao: z.string({
				required_error: "Date de inserção da pendência não informada.",
			}),
		}),
	),
	anotacoes: z.string({
	...
	tipoSolicitacao: z.enum(
		[
			TechnicalAnalysisSolicitationTypes[0].value,
			...TechnicalAnalysisSolicitationTypes.slice(1).map((p) => p.value),
		],
		{
			required_error: "Tipo da solicitação de análise não informada.",
			invalid_type_error:
				"Tipo não válido para o tipo de solicitação da análise.",
		},
	),
	comentarios: z
		.string({
			invalid_type_error: "Tipo não válido para os comentários da análise.",
		})
		.optional()
		.nullable(),
	analista: z
		.object({
			id: z.union([z.number(), z.string()], {
				...
				invalid_type_error:
					"Tipo não válido para o ID de referência do analista.",
			}),
			nome: z.string({
				required_error: "Nome do analista não informado.",
				invalid_type_error: "Tipo não válido para o nome do analista.",
			}),
			apelido: z.string({
				required_error: "Apelido do analista não informado.",
				invalid_type_error: "Tipo não válido para o apelido do analista.",
			}),
			avatar_url: z
				.string({
					invalid_type_error: "Tipo não válido para o avatar do analista.",
				})
				.optional()
				.nullable(),
		})
	...
	oportunidade: z.object({
		...
		identificador: z
			.string({
				required_error:
					"Identificador da oportunidade de referência não informado.",
				invalid_type_error:
					"Tipo não válido para o identificador da oportunidade de referência não informado.",
			})
			.optional()
			.nullable(),
	}),
	localizacao: z.object({
		cep: z.string({
			...
			invalid_type_error:
				"Tipo não válido para o CEP da localização de análise.",
		}),
		uf: z
			.string({
				required_error: "UF da localização de análise não informada.",
				invalid_type_error:
					"Tipo não válido para o UF da localização de análise.",
			})
			.optional()
			.nullable(),
		cidade: z
			.string({
				...
				invalid_type_error:
					"Tipo não válido para o cidade da localização de análise.",
			})
			.optional()
			.nullable(),
		bairro: z.string({
			...
			invalid_type_error:
				"Tipo não válido para o bairro da localização de análise.",
		}),
		endereco: z.string({
			required_error: "Endereço da localização de análise não informada.",
			invalid_type_error:
				"Tipo não válido para o endereço da localização de análise.",
		}),
		numeroOuIdentificador: z.string({
			required_error:
				"Número ou identificador da localização de análise não informada.",
			invalid_type_error:
				"Tipo não válido para o número ou identificador da localização de análise.",
		}),
	}),
	...
	padrao: z.array(
		z.object({
			alteracao: z.boolean({
				...
				invalid_type_error:
					"Tipo não válido para a necessidade de alteração do padrão.",
			}),
			tipo: z.union(
				[z.literal("CONTRA À REDE"), z.literal("À FAVOR DA REDE")],
				{
			...
			ligacao: z.string({
				required_error: "Ligação do padrão não informada.",
				invalid_type_error: "Tipo não válido para a ligação do padrão.",
			}), //
			novaAmperagem: z
				.union([z.string(), z.number()], {
					invalid_type_error:
						"Tipo não válido para a nova amperagem do padrão.",
				})
				.optional()
				.nullable(), // x
			novaLigacao: z
				.string({
					invalid_type_error: "Tipo não válido para a nova ligação do padrão.",
				})
				.optional()
				.nullable(), // x
			codigoMedidor: z.string({
				...
				invalid_type_error:
					"Tipo não válido para o código do medidor do padrão.",
			}),
			modeloCaixaMedidor: z
				.string({
					invalid_type_error:
						"Tipo não válido para o modelo da caixa do medidor.",
				})
				.optional()
				.nullable(),
			codigoPosteDerivacao: z
				.string({
					invalid_type_error:
						"Tipo não válido para o código do poste de derivação.",
				})
				.optional()
				.nullable(), // GO only
		}),
	...
	transformador: z.object({
		acopladoPadrao: z.boolean({
			...
			invalid_type_error:
				"Tipo não válido para o status de acoplação do padrão.",
		}),
		codigo: z.string({
	...
	execucao: z.object({
		observacoes: z
			.string({
				required_error: "Observações de execução não informadas.",
				invalid_type_error: "Tipo não válido para as observações da execução.",
			})
			.optional()
			.nullable(),
		memorial: z
			.array(
				z.object({
					topico: z.string({
						...
						invalid_type_error:
							"Tipo não válido para o tópico do memorial de execução não informado.",
					}), // avaliar telhado, adaptar qgbt, etc
					descricao: z.string({
						required_error: "Descrição do memorial de execução não informado.",
						invalid_type_error:
							"Tipo não válido para o descrição do memorial de execução não informado.",
					}),
				}),
			)
			.optional()
	...
	descritivo: z.array(
		z.object({
			topico: z.string({
				required_error: "Tópico do descritivo não informado.",
				invalid_type_error: "Tipo não válido para o tópico do descritivo.",
			}),
			descricao: z.string(),
		}),
	),
	servicosAdicionais: z.object({
		alambrado: z
			.union(
				[
					z.literal("NÃO"),
					z.literal("SIM - RESPONSABILIDADE DO CLIENTE"),
					z.literal("SIM - RESPONSABILIDADE DA EMPRESA"),
				],
				{
					invalid_type_error:
						"Tipo não válido para a especificação de execução de alambrado.",
				},
			)
			.optional()
			.nullable(),
		britagem: z
			.union(
				...
				{
					invalid_type_error:
						"Tipo não válido para a especificação de execução de britagem.",
				},
			)
			.optional()
			.nullable(),
		casaDeMaquinas: z
			.union(
				...
				{
					invalid_type_error:
						"Tipo não válido para a especificação de execução de casa de máquinas.",
				},
			)
			.optional()
			.nullable(),
		barracao: z
			.union(
				...
				{
					invalid_type_error:
						"Tipo não válido para a especificação de execução de barracão.",
				},
			)
			.optional()
			.nullable(),
		roteador: z
			.union(
				...
				{
					invalid_type_error:
						"Tipo não válido para a especificação de execução de instação de roteador.",
				},
			)
			.optional()
			.nullable(),
		limpezaLocal: z
			.union(
				...
				{
					invalid_type_error:
						"Tipo não válido para a especificação de execução de limpeza do local.",
				},
			)
			.optional()
			.nullable(),
		redeReligacao: z
			.union(
				...
				{
					invalid_type_error:
						"Tipo não válido para a especificação de execução de rede de religação.",
				},
			)
			.optional()
			.nullable(),
		terraplanagem: z
			.union(
				...
				{
					invalid_type_error:
						"Tipo não válido para a especificação de execução de terraplagem.",
				},
			)
			.optional()
			.nullable(),
		realimentar: z.boolean({
			...
			invalid_type_error:
				"Tipo não válido para o status da necessidade de realimentação.",
		}),
	}),
	detalhes: z.object({
		...
		topologia: z
			.union([z.literal("MICRO-INVERSOR"), z.literal("INVERSOR")], {
				invalid_type_error: "Tipo não válido para a topologia.",
			})
			.optional()
			.nullable(),
		materialEstrutura: z
			.union([z.literal("MADEIRA"), z.literal("FERRO")], {
				invalid_type_error: "Tipo não válido para o material da estrutura.",
			})
			.optional()
			.nullable(),
		tipoEstrutura: z
		...
		imagensDrone: z.boolean({
			...
			invalid_type_error:
				"Tipo não válido para o status da existência de imagens de drone.",
		}),
		imagensFachada: z.boolean({
			required_error:
				"Status da existência de imagens da fachada não informada.",
			invalid_type_error:
				"Tipo não válido para o status da existência de imagens da fachada.",
		}),
		imagensSatelite: z.boolean({
			required_error:
				"Status da existência de imagens de satélite não informada.",
			invalid_type_error:
				"Tipo não válido para o status da existência de imagens de satélite.",
		}),
		medicoes: z.boolean({
			required_error:
				"Status da existência de imagens de medições não informada.",
			invalid_type_error:
				"Tipo não válido para o status da existência de imagens de medições.",
		}),
		orientacao: z.string({
		...
		telhasReservas: z
			.union([z.literal("NÃO"), z.literal("SIM")], {
				...
				invalid_type_error:
					"Tipo não válido para a existência de telhas reservas.",
			})
			.optional()
			.nullable(),
	}),
	distancias: z.object({
		conexaoInternet: z.string({
			required_error:
				"Distância do sistema a um ponto de internet não informada.",
			invalid_type_error:
				"Tipo não válido para a distância do sistema a um ponto de internet.",
		}),
	...
	locais: z.object({
		aterramento: z
			.string({
				required_error: "Local de aterramento não informado.",
				invalid_type_error: "Tipo não válido para o local de aterramento.",
			})
			.optional()
			.nullable(),
		inversor: z.string({
			...
			invalid_type_error:
				"Tipo não válido para o local de instalação do inversor.",
		}), // instalação do inversor, varannda, garagem, etc
		modulos: z.string({
			required_error: "Local de instalação dos módulos não informado.",
			invalid_type_error:
				"Tipo não válido para o local de instalação dos módulos.",
	...
	custos: z.array(
		z.object({
			categoria: z
				...
				.nullable(),
			descricao: z.string({
				required_error: "Descrição do custo não informada.",
				invalid_type_error: "Tipo não válido para a descrição do custo.",
			}),
			qtde: z.number({
				required_error: "Quantidade do item de custo não informada.",
				invalid_type_error:
					"Tipo não válido para a quantidade do item de custo.",
			}),
			grandeza: z.string({
				required_error: "Grandeza do custo não informada.",
				invalid_type_error: "Tipo não válido para a grandeza do custo.",
			}),
			custoUnitario: z
				.number({
					invalid_type_error:
						"Tipo não válido para o custo unitário do item de custo.",
				})
				.optional()
				.nullable(),
			total: z
				.number({
					invalid_type_error: "Tipo não válido para o total do item de custo.",
				})
				.optional()
				.nullable(),
			totalVendaSimples: z
				.number({
					invalid_type_error:
						"Tipo não válido para o total do item para venda simples.",
				})
				.optional()
				.nullable(),
			totalVendaFaturavel: z
				.number({
					invalid_type_error:
						"Tipo não válido para o total do item para venda faturável.",
				})
				.optional()
				.nullable(),
		}),
	...
	alocacaoModulos: z.object({
		leste: z
			.number({
				invalid_type_error:
					"Tipo não válido para quantidade alocada de modelo para leste.",
			})
			.optional()
			.nullable(),
		nordeste: z
			.number({
				invalid_type_error:
					"Tipo não válido para quantidade alocada de modelo para nordeste.",
			})
			.optional()
			.nullable(),
		noroeste: z
			.number({
				invalid_type_error:
					"Tipo não válido para quantidade alocada de modelo para noroeste.",
			})
			.optional()
			.nullable(),
		norte: z
			.number({
				invalid_type_error:
					"Tipo não válido para quantidade alocada de modelo para norte.",
			})
			.optional()
			.nullable(),
		oeste: z
			.number({
				invalid_type_error:
					"Tipo não válido para quantidade alocada de modelo para oeste.",
			})
			.optional()
			.nullable(),
		sudeste: z
			.number({
				invalid_type_error:
					"Tipo não válido para quantidade alocada de modelo para sudeste.",
			})
			.optional()
			.nullable(),
		sudoeste: z
			.number({
				invalid_type_error:
					"Tipo não válido para quantidade alocada de modelo para sudoeste.",
			})
			.optional()
			.nullable(),
		sul: z
			.number({
				invalid_type_error:
					"Tipo não válido para quantidade alocada de modelo para sul.",
			})
			.optional()
			.nullable(),
	}),
	desenho: z.object({
		...
		tipo: z
			.string({ invalid_type_error: "Tipo não válido para o tipo de desenho." })
			.optional()
			.nullable(), //
		url: z
			.string({ invalid_type_error: "Tipo não válido para a URL do desenho." })
			.optional()
			.nullable(),
	}),
	suprimentos: z
			...
			itens: z.array(
				z.object({
					descricao: z.string({
						...
						invalid_type_error:
							"Tipo não válido para a descrição do item de suprimentação.",
					}),
					tipo: z.string({
						required_error: "Tipo do item de suprimentação não informada.",
						invalid_type_error:
							"Tipo não válido para o tipo do item de suprimentação.",
					}),
					qtde: z.number({
						required_error:
							"Quantidade do item de suprimentação não informada.",
						invalid_type_error:
							"Tipo não válido para a quantidade do item de suprimentação.",
					}),
					grandeza: z.string({
						required_error: "Grandeza do item de suprimentação não informada.",
						invalid_type_error:
							"Tipo não válido para a grandeza do item de suprimentação.",
					}),
				}),
			),
		})
	...
	conclusao: z.object({
		...
		espaco: z.boolean({
			required_error:
				"Status do espaço para execução do serviço não informado.",
			invalid_type_error:
				"Tipo não válido para o status do espaço para execução do serviço.",
		}), // possui espaço pra execução
		inclinacao: z.boolean({
			required_error: "Necessidade de estrutura de inclinação não informada.",
			invalid_type_error:
				"Tipo não válido para necessidade de estrutura de inclinação.",
		...
		padrao: z
			.union([z.literal("APTO"), z.literal("REFORMAR"), z.literal("TROCAR")], {
				invalid_type_error:
					"Tipo não válido para conclusão da condição do padrão de energia.",
			})
			.optional()
			.nullable(),
		estrutura: z
			.union(
				[
					...
					z.literal("CONDENADO"),
					z.literal("REFORÇAR"),
					z.literal("AVALIAR NA EXECUÇÃO"),
				],
				{
					invalid_type_error:
						"Tipo não válido para a condição da estrutura de montagem.",
				},
			)
			.optional()
			.nullable(),
	...
	dataEfetivacao: z
		.string({
			invalid_type_error:
				"Tipo não válido para a data de efetivação da análise técnica.",
		})
		.datetime({
			message: "Formato inválido para a data de efetivação da análise.",
		})
		.optional()
		.nullable(),
	dataInsercao: z
...
export const PersonalizedTechnicalAnalysisFiltersSchema = z.object({
	...
	status: z.array(
		z.string({
			required_error: "Status de filtro não informada.",
	...
	type: z.array(
		...
		}),
		{
			...
			invalid_type_error:
				"Tipo não válido para lista de filtro de tipos de solicitação.",
		},
	),
...
export type TPersonalizedTechnicalAnalysisFilter = z.infer<
	typeof PersonalizedTechnicalAnalysisFiltersSchema
>;
export const PersonalizedTechnicalAnalysisQuerySchema = z.object({
	applicants: z
		.array(
			z.string({
				required_error: "Requerentes não informados ou inválidos.",
				invalid_type_error: "Requerentes inválidos.",
			}),
		)
		.nullable(),
	partners: z
	...
	analysts: z
		.array(
			z.string({
				required_error: "Analistas não informados ou inválidos.",
				invalid_type_error: "Analistas inválidos.",
			}),
		)
		.nullable(),
...
export type StructureTypesFromAnalysis =
	TTechnicalAnalysis["detalhes"]["tipoEstrutura"];
