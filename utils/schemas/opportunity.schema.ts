...
export const GeneralOpportunitySchema = z.object({
	...
	ganho: z.object({
		...
		dataSolicitacao: z
			.string()
			.datetime({
				message: "Formato inválido para data de solicitação de contrato.",
			})
			.optional()
			.nullable(),
	}),
	...
	ultimaInteracao: z
		.object({
			...
			data: z
				.string({
					invalid_type_error: "Tipo não válido para data de última interação.",
				})
...
export const OpportunityWithClientSchema = z.object({
	...
	cliente: GeneralClientSchema.extend({
		_id: z.string({
			required_error: "ID de referência do cliente não informado.",
			invalid_type_error: "Tipo não válido para ID de referência do cliente.",
		}),
