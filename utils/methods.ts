...
export function getEstimatedGen(peakPower: number, city: string | undefined | null, uf: string | undefined | null, orientation?: (typeof orientations)[number]): number {
...
export function getInverterStr(inverters: InverterType[], kitType: string | undefined) {
	...
	if (kitType == "PROMOCIONAL") {
		...
		for (let i = 0; i < inverters.length; i++) {
			if (i < inverters.length - 1) {
				str = str + `${inverters[i].qtde}x ${inverters[i].fabricante != "PERSONALIZADO" ? inverters[i].fabricante : inverters[i].modelo} (${inverters[i].modelo}) & `;
			} else {
				str = str + `${inverters[i].qtde}x ${inverters[i].fabricante != "PERSONALIZADO" ? inverters[i].fabricante : inverters[i].modelo} (${inverters[i].modelo})`;
			}
		}
