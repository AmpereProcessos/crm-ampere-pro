...
import type { TSalePromoterResultsById, TSalePromotersResults } from "@/app/api/stats/comercial-results/sellers/route";
...
async function fetchSalePromotersResults({ after, before }: FetchSalePromotersResultsParams) {
	try {
		...
		throw new Error("Erro ao buscar resultados dos promotores de vendas");
	}
}
...
async function fetchSalePromoterResultsById({ id, firstPeriodAfter, firstPeriodBefore, secondPeriodAfter, secondPeriodBefore }: UseSalePromoterResultsByIdParams) {
	try {
		...
		throw new Error("Erro ao buscar resultados do promotor de vendas");
	}
}
