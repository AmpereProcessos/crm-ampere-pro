...
type ProjectFileAttachmentMenuProps = {
  projectId: string
  clientId: string
...
function ProjectFileAttachmentMenu({ projectId, clientId, opportunityId, analysisId, homologationId, session }: ProjectFileAttachmentMenuProps) {
  ...
  async function handleFileReferenceCreation() {
    try {
      ...
      const vinculationId = attachmentHolder.idProject || attachmentHolder.idOpportunity || attachmentHolder.idClient || 'naodefinido'
  ...
  const { mutate: handleAttach, isPending } = useMutationWithFeedback({
    ...
    affectedQueryKey: ['file-references-by-query', clientId, opportunityId, analysisId, homologationId, projectId],
    callbackFn: () =>
      setAttachmentHolder({
        title: '',
        files: null,
      ...
      <div className="my-4 flex w-full flex-col gap-2">
        ...
        {newFileMenuIsOpen ? (
          <motion.div
            ...
            animate="visible"
            exit="exit"
            className="mt-2 flex w-[90%] flex-col gap-2 self-center rounded border border-gray-500 p-6 lg:flex-row"
          >
              ...
              <div className="mt-4 flex w-full flex-col flex-wrap items-center justify-center gap-4 lg:flex-row">
                ...
                <div className="w-fit">
                  <CheckboxInput
                    ...
                    justify="justify-center"
                    handleChange={(value) => setAttachmentHolder((prev) => ({ ...prev, idOpportunity: !!prev.idOpportunity ? undefined : opportunityId }))}
                  />
                </div>
              </div>
...
export default ProjectFileAttachmentMenu
