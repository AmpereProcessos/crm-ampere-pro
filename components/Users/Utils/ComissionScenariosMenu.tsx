    ...
    <div className='w-full flex flex-col gap-2'>
      <div className='w-full flex flex-col gap-1'>
        ...
        <p className='text-xs text-muted-foreground'>
          Configure as comissões para cada cenário. Com base nos tipos de projetos, defina as comissões do usuário para cada papel desempenhado na
          venda.
        </p>
      </div>
...
function ComissionProjectTypeCard({
  projectType,
  userComissionConfig,
  addComissionConfigItem,
  updateComissionConfigItem,
  partners,
}: ComissionProjectTypeCardProps) {
  const [newComissionMenuActiveRole, setNewComissionMenuActiveRole] = useState<string | null>(null);
  const projectTypeComissionConfig = userComissionConfig
    .map((c, i) => ({ ...c, itemIndex: i }))
    .filter((scenario) => scenario.tipoProjeto.id === projectType._id);
  const missingRoleComissionConfig = OpportunityResponsibilityRoles.filter(
    (role) => !projectTypeComissionConfig.some((scenario) => scenario.papel === role.value)
  ).map((role) => role.value);
  function addComissionConfigItemResult(info: { comissionConfigIndex: number; result: TUser['comissionamento'][number]['resultados'][number] }) {
    if (info.result.condicao.aplicavel) {
      ...
      if (info.result.condicao.tipo === 'IGUAL_TEXTO' && !info.result.condicao.igual)
        return toast.error('Selecione o resultado para comparação da condição.');
      ...
      if (
        info.result.condicao.tipo === 'MAIOR_QUE_NÚMERICO' &&
        (info.result.condicao.maiorQue === null || info.result.condicao.maiorQue === undefined)
      )
        return toast.error('Preencha o valor para comparação.');
      if (
        info.result.condicao.tipo === 'MENOR_QUE_NÚMERICO' &&
        (info.result.condicao.menorQue === null || info.result.condicao.menorQue === undefined)
      )
        return toast.error('Preencha o valor para comparação.');
      if (
  ...
  function updateComissionConfigItemResult(info: {
    comissionConfigIndex: number;
    resultIndex: number;
      ...
      {projectTypeComissionConfig
        ? projectTypeComissionConfig.map((scenario, index) => (
            <ComissionProjectTypeComissionConfigCard
              ...
              updateComissionConfigItemResult={(data) =>
                updateComissionConfigItemResult({ comissionConfigIndex: scenario.itemIndex, resultIndex: data.resultIndex, result: data.result })
              }
              partners={partners}
            />
          ))
  ...
  return (
    <div className='flex w-full flex-col gap-3 file:p-2'>
      ...
      {newResultModalIsOpen ? (
        <ComissionResultModal
          handleCommitConditionResult={(info) => addComissionConfigItemResult(info)}
          closeModal={() => setNewResultModalIsOpen(false)}
        />
      ) : null}
    </div>
  );
...
function ComissionProjectTypeCardResult({ result, updateResult, partners }: ComissionProjectTypeCardResultProps) {
  ...
  return (
    <div className='w-full flex flex-col gap-1 rounded-sm border border-primary/30 shadow-2xs p-2'>
      <div className='flex w-full flex-col items-center justify-between gap-2 lg:flex-row'>
        <div className='flex flex-wrap items-center gap-2'>
          {handleRenderComissionScenarioResultConditionPhrase({
            result: result,
      ...
      {editModalIsOpen ? (
        <ComissionResultModal
          ...
          handleCommitConditionResult={(info) => updateResult(info)}
