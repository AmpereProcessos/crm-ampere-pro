...
import type { TUser, TUserComissionItem } from "@/utils/schemas/user.schema";
import {
...
function ComissionResultModal({ initialResult, handleCommitConditionResult, closeModal }: ComissionResultModalProps) {
	...
	return isDesktop ? (
		<Dialog open={true} onOpenChange={(v) => (!v ? closeModal() : null)}>
			<DialogContent className="min-w-[80%] w-[80%] h-[85vh] dark:bg-white">
				<DialogHeader>
...
function CalculationBlock({ resultHolder, updateHolder }: CalculationBlockProps) {
	...
	return (
		<div className="flex w-full flex-col gap-2">
			...
			<div className="h-[1px] w-full rounded-lg bg-primary" />
			...
			<div className="flex w-full flex-col gap-2">
				...
				<p className="w-full text-start text-sm font-light tracking-tighter text-primary/80">Define aqui alguns parâmetros para o cenário de comissão.</p>
							...
							<div className="flex w-full flex-col gap-2 lg:flex-row">
								<div className="w-full lg:w-1/2">
									<NumberInput
										...
										value={resultHolder.condicao.entre !== null && resultHolder.condicao.entre !== undefined ? Number(resultHolder.condicao.entre) : null}
										handleChange={(value) =>
											updateHolder({
												condicao: { ...resultHolder.condicao, entre: { ...resultHolder.condicao.entre, minimo: value, maximo: resultHolder.condicao.entre?.maximo || 0 } },
											})
										}
										width="100%"
									/>
								...
								<div className="w-full lg:w-1/2">
									<NumberInput
										...
										handleChange={(value) =>
											updateHolder({
												condicao: { ...resultHolder.condicao, entre: { ...resultHolder.condicao.entre, minimo: resultHolder.condicao.entre?.minimo || 0, maximo: value } },
											})
										}
										width="100%"
									/>
	...
	return (
		<>
			...
			<div className="flex w-full flex-col gap-2">
				...
				<p className="w-full text-start text-sm font-light tracking-tighter text-primary/80">Preencha aqui um número e clique no botão de adicionar para adicionar à fórmula.</p>
			...
			<div className="flex w-full flex-col gap-2">
				...
				<p className="w-full text-start text-sm font-light tracking-tighter text-primary/80">Clique em uma das premissas númericas da venda para adiciona-la à fórmula.</p>
				<div className="my-2 flex flex-wrap items-center gap-2">
					{NumericalSaleDefinitions.map((va, index) => (
						<button
							...
							className="grow rounded border border-primary/80 px-4 py-1 text-[0.65rem] font-medium text-primary/80 duration-300 ease-in-out hover:bg-primary/80 hover:text-primary-foreground"
						>
							{va.label}
						</button>
					))}
			...
			<div className="flex w-full flex-col gap-2">
				...
				<p className="w-full text-start text-sm font-light tracking-tighter text-primary/80">Clique em um dos operadores matemáticos para adiciona-lo à fórmula.</p>
				<div className="my-2 flex flex-wrap items-center gap-2">
					{MathematicalOperators.map((va, index) => (
						<button
							...
							className="text-medium grow rounded border border-primary/80 px-4 py-1 text-[0.65rem] font-medium text-primary/80 duration-300 ease-in-out hover:bg-primary/80 hover:text-primary-foreground"
						>
							{va.value}
						</button>
					))}
