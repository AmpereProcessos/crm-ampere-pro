		...
		<div id="edit-proposal" className="fixed bottom-0 left-0 right-0 top-0 z-[100] bg-[rgba(0,0,0,.85)]">
			<div className="fixed left-[50%] top-[50%] z-[100] flex h-fit max-h-[90%] w-[90%] translate-x-[-50%] translate-y-[-50%] flex-col rounded-md bg-[#fff] p-[10px] lg:w-[90%]">
				...
				<div className="flex grow flex-col gap-y-2 overflow-y-auto overscroll-y-auto px-2 py-1 scrollbar-thin scrollbar-track-gray-100 scrollbar-thumb-gray-300">
					<TextInput label="NOME DA PROPOSTA" placeholder="Preencha o nome da proposta..." value={proposalName} handleChange={(value) => setProposalName(value)} width="100%" />
					...
					<div className="flex w-full items-center justify-end gap-2 p-2">
						{info.idModeloAnvil ? (
							<div className="w-fit">
								<CheckboxInput labelFalse="GERAR NOVO DOCUMENTO" labelTrue="GERAR NOVO DOCUMENTO" checked={regenerateFile} handleChange={(value) => setRegenerateFile(value)} />
							</div>
						) : null}
						<button
							disabled={isPending}
			...
			{addNewPriceItemModalIsOpen ? <AddPricingItem pricing={pricing} setPricing={setPricing} proposal={info} closeModal={() => setAddNewPriceItemModalIsOpen(false)} /> : null}
...
function ProposalPaymentMethods({ proposalValue, payment, setPayment }: ProposalPaymentMethodsProps) {
	...
	return (
		<div className="flex w-full flex-col gap-2 rounded border border-[#fead41]">
			...
			<p className="my-2 w-full text-center font-medium leading-none tracking-tight text-gray-500">Selecione os métodos de pagamento aplicáveis a essa proposta.</p>
			<div className="flex w-full flex-col gap-2 p-2">
				...
				{isSuccess ? (
					paymentMethods.length > 0 ? (
						...
						<p className="flex w-full grow items-center justify-center py-2 text-center font-medium italic tracking-tight text-gray-500">Nenhum método de pagamento disponível.</p>
					)
				) : null}
			</div>
		</div>
