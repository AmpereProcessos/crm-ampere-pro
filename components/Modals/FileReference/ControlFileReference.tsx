...
import { getFileTypeTitle, getTitleFileType, isFileImage, uploadFile } from '@/lib/methods/firebase';
...
function ControlFileReference({ fileReferenceId, vinculations, session, affectedQueryKey, closeModal }: ControlFileReferenceProps) {
  ...
  async function handleUpdateFileReference({ fileReference, attachment }: { fileReference: TFileReference; attachment: TAttachmentHolder | null }) {
    try {
      let fileMetadata = {
  ...
  return (
    <div id='control-file-reference' className='fixed bottom-0 left-0 right-0 top-0 z-100 bg-[rgba(0,0,0,.85)]'>
      <div className='fixed left-[50%] top-[50%] z-100 h-[60%] w-[90%] translate-x-[-50%] translate-y-[-50%] rounded-md bg-background p-[10px] lg:w-[40%]'>
        <div className='flex h-full flex-col'>
          <div className='flex flex-wrap items-center justify-between border-b border-primary/30 px-2 pb-2 text-lg'>
            <h3 className='text-xl font-bold text-primary'>EDITAR ANEXOS</h3>
            <button
              onClick={() => closeModal()}
              type='button'
          ...
          {isSuccess ? (
            <>
              <div className='flex grow flex-col overflow-x-hidden overflow-y-auto overscroll-y-auto py-4 gap-4 scrollbar-thin scrollbar-track-primary/10 scrollbar-thumb-primary/30'>
                <TextInput
                ...
                />
                <ControlFileReferenceFileAttachment
                  fileReference={infoHolder}
                  attachmentHolder={fileHolder}
                  updateAttachment={(info) => setFileHolder(info)}
                />
              </div>
              <div className='flex w-full flex-col items-center justify-between gap-2 lg:flex-row px-2'>
                <LoadingButton
                  ...
                  onClick={() => mutateDeleteFileReference({ id: fileReferenceId })}
                  type='button'
                >
                  REMOVER ANEXO
                </LoadingButton>
...
function ControlFileReferenceFileAttachment({ fileReference, attachmentHolder, updateAttachment }: ControlFileReferenceFileAttachmentProps) {
  ...
  return (
    <div className='flex w-full flex-col gap-2'>
      <div className='flex h-[200px] w-[200px] flex-col self-center rounded-sm border border-primary/50'>
        <div className='relative flex h-[200px] w-full grow items-center justify-center bg-linear-to-b from-sky-400 to-sky-200'>
          {imageUrl ? (
