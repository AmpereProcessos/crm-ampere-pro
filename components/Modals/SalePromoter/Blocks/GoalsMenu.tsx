...
function GoalsMenu({ promoter, session }: GoalsMenuProps) {
	...
	return (
		<div className="flex w-full flex-col gap-2">
			<h1 className="w-full rounded bg-black p-1 text-center text-xs font-medium text-white">METAS</h1>
			<div className="flex w-full items-center justify-end">
				<button
					...
					className={cn("flex items-center gap-1 rounded-lg px-2 py-1 text-black duration-300 ease-in-out", {
						"bg-gray-300  hover:bg-red-300": newSaleGoalMenuIsOpen,
						"bg-green-300  hover:bg-green-400": !newSaleGoalMenuIsOpen,
					})}
				>
					<Goal />
					<h1 className="text-xs font-medium tracking-tight">{!newSaleGoalMenuIsOpen ? "ABRIR MENU DE NOVA META DE VENDAS" : "FECHAR MENU DE NOVA META DE VENDAS"}</h1>
				</button>
			</div>
			<AnimatePresence>{newSaleGoalMenuIsOpen ? <NewSaleGoalMenu promoter={promoter} session={session} closeMenu={() => setNewSaleGoalMenuIsOpen(false)} /> : null}</AnimatePresence>
			...
			<div className="flex w-full flex-col gap-2">
				...
				{isSuccess ? (
					saleGoals.length > 0 ? (
						...
						<p className="w-full text-center text-xs font-medium tracking-tight text-gray-500">Nenhuma meta encontrada para o promotor de vendas.</p>
					)
				) : null}
			</div>
		</div>
...
function NewSaleGoalMenu({ promoter, session, closeMenu }: NewSaleGoalMenuProps) {
	...
	return (
		<motion.div
			...
			animate="visible"
			exit="exit"
			className="flex w-full flex-col gap-2 overflow-hidden rounded border border-green-600"
		>
...
function SaleGoalCard({ goal }: SaleGoalCardProps) {
	...
	return (
		<div className="flex w-full flex-col gap-1 rounded border border-black bg-[#fff] p-2 shadow-sm dark:bg-[#121212]">
			...
			<h1 className="w-full py-0.5 text-start text-[0.6rem] font-medium italic text-gray-500">METAS DEFINIDAS</h1>
			...
			<div className="flex w-full flex-col justify-between gap-2 lg:flex-row">
				<div className="flex w-full items-center justify-center gap-1 lg:w-1/2 lg:justify-start">
					...
					<h1 className="text-primary py-0.5 text-center text-[0.6rem] font-bold">{goal.metas.potenciaVendida ? `${formatDecimalPlaces(goal.metas.potenciaVendida)}kWp` : "N/A"}</h1>
				</div>
				<div className="flex w-full flex-row items-center justify-center gap-1 lg:w-1/2 lg:flex-row-reverse lg:justify-start">
					...
					<h1 className="text-primary py-0.5 text-center text-[0.6rem] font-bold">{goal.metas.valorVendido ? `${formatToMoney(goal.metas.valorVendido)}` : "N/A"}</h1>
				</div>
			</div>
			<div className="flex w-full flex-col justify-between gap-2 lg:flex-row">
				<div className="flex w-full items-center justify-center gap-1 lg:w-1/2 lg:justify-start">
				...
				<div className="flex w-full flex-row items-center justify-center gap-1 lg:w-1/2 lg:flex-row-reverse lg:justify-start">
					...
					<h1 className="text-primary py-0.5 text-center text-[0.6rem] font-bold">{goal.metas.conversao ? `${formatDecimalPlaces(goal.metas.conversao)}%` : "N/A"}</h1>
				</div>
			</div>
			<div className="flex w-full items-center justify-end gap-2">
				...
				<button onClick={() => setEditGoalMenuIsOpen((prev) => !prev)} className="flex items-center gap-1 rounded-lg bg-black px-2 py-1 text-[0.6rem] text-white">
			...
			<AnimatePresence>
				{editGoalMenuIsOpen ? (
					<motion.div
						...
						animate="visible"
						exit="exit"
						className="flex flex-col gap-2 self-center overflow-hidden rounded border border-blue-600"
					>
