...
function getRequestObjectByProjectType({ propose, projectType, responsible, client }: GetRequestObjectByProjectTypeArguments): TContractRequest {
  ...
  return {
    ...
    planoOeM:
      propose.oportunidadeDados.tipo.titulo === 'OPERAÇÃO E MANUTENÇÃO' ? (propose.planos[0]?.nome as TContractRequest['planoOeM']) : 'NÃO SE APLICA',
    clienteSegurado: 'NÃO',
      ...
      <div className='fixed left-[50%] top-[50%] z-100 h-[80%] w-[90%] translate-x-[-50%] translate-y-[-50%] rounded-md bg-background p-[10px] lg:w-[70%]'>
        <div className='flex h-full flex-col'>
          <div className='flex flex-col items-center justify-between border-b border-primary/30 px-2 pb-2 text-lg lg:flex-row'>
            <h3 className='text-xl font-bold text-primary'>SOLICITAÇÃO DE CONTRATO</h3>
            <button
              onClick={closeModal}
              type='button'
          ...
          <div className='flex h-full flex-col gap-y-2 overflow-y-auto overscroll-y-auto p-2 py-1 scrollbar-thin scrollbar-track-primary/10 scrollbar-thumb-primary/30'>
