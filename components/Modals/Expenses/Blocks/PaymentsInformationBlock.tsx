...
function PaymentsInformationBlock({ infoHolder, setInfoHolder }: PaymentsInformationBlockProps) {
  ...
  return (
    <div className='flex w-full flex-col gap-y-2'>
      <h1 className='w-full bg-primary/70  p-1 text-center font-medium text-primary-foreground'>INFORMAÇÕES DE PAGAMENTO</h1>
      <div className='flex w-full flex-col gap-1'>
        ...
        <h1 className='w-full bg-primary/50 p-1 text-center text-xs font-medium text-primary-foreground'>PAGAMENTOS DA DESPESA</h1>
        <div className='flex w-full flex-col gap-2'>
          <div className='flex w-full flex-col items-center gap-2 lg:flex-row'>
            ...
            <div className='w-full lg:w-[25%]'>
              <DateInput
                ...
                value={formatDateForInputValue(paymentHolder.dataPagamento)}
                handleChange={(value) =>
                  setPaymentHolder((prev) => ({
                    ...prev,
            ...
            <div className='flex w-full items-center justify-center lg:w-[25%]'>
              <div className='w-fit'>
                <CheckboxInput
                  labelFalse='PAGO'
                  labelTrue='PAGO'
                  checked={paymentHolder.efetivado}
                  handleChange={(value) => setPaymentHolder((prev) => ({ ...prev, efetivado: value }))}
                />
              </div>
            </div>
          </div>
          <div className='flex items-center justify-end'>
            <button
              ...
              onClick={() => addPayment(paymentHolder)}
            >
              ADICIONAR PAGAMENTO
            </button>
          </div>
