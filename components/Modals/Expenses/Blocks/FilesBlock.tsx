...
import ExpenseFileAttachmentMenu from "./Utils/FileAttachmentMenu";
...
function FilesBlock({ expenseId, revenueId, projectId, clientId, opportunityId, analysisId, homologationId, purchaseId, session }: FilesBlockProps) {
	const {
		data: fileReferences,
		isLoading,
		isError,
		isSuccess,
	} = useFileReferences({ projectId, clientId, opportunityId, analysisId, homologationId, purchaseId, revenueId, expenseId });
	return (
		<div className="flex w-full flex-col gap-2 rounded border border-gray-800">
			...
			<ExpenseFileAttachmentMenu expenseId={expenseId} purchaseId={purchaseId} projectId={projectId} clientId={clientId} opportunityId={opportunityId} session={session} />
		</div>
	);
}
