...
type ExpenseCompositionTableProps = {
  infoHolder: TExpense;
  setInfoHolder: React.Dispatch<React.SetStateAction<TExpense>>;
};
function ExpenseCompositionTable({ infoHolder, setInfoHolder }: ExpenseCompositionTableProps) {
  function updateItem({ item, index }: { item: TExpenseCompositionItem; index: number }) {
      ...
      <div className='flex w-full flex-col gap-2 p-1'>
        {infoHolder.composicao.length > 0 ? (
          ...
          <p className='flex w-full grow items-center justify-center py-2 text-center text-sm font-medium italic tracking-tight text-primary/70'>
            Não há itens de composição da despesa.
          </p>
        )}
      </div>
    </div>
...
export default ExpenseCompositionTable;
