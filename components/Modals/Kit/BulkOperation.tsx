...
import {
  CommonProductsByProjectType,
  CommonServicesByProjectType,
...
function KitBulkOperation({ session, closeModal }: KitBulkOperationProps) {
  ...
  const [kitsHolder, setKitsHolder] = useState<
    (TKit & { _id: string | null; excluir: boolean })[]
  >([]);
  async function handleExtracting(file: File | null) {
    ...
    try {
      ...
      const bulkOperationKits = await z
        .array(KitsBulkOperationItemSchema)
        .parseAsync(data);
      // Formatting data into TKit schema
      const kits: (TKit & { _id: string | null; excluir: boolean })[] =
        bulkOperationKits.map((bulkKit) => {
          const productsArr = [];
          const servicesArr = [];
          ...
          const pricingMethodologyId =
            pricingMethods?.find(
              (m) => m.nome == bulkKit["METODOLOGIA DE PRECIFICAÇÃO"]
            )?._id || "660dab0b0fcb72da4ed8c35e";
          // Finding the equivalent defined partner, if not found, using users partner
          const definedPartner =
            bulkKit["VISIBILIDADE DE PARCEIRO"] != "N/A"
              ? partners?.find(
                  (p) => p.nome == bulkKit["VISIBILIDADE DE PARCEIRO"]
                )?._id || userPartnerId
              : null;
          ...
          const modulesTotalPeakPower =
            getModulesPeakPotByProducts(productsArr);
          ...
          const dueDate = bulkKit["DATA DE VALIDADE"]
            ? isNaN(bulkKit["DATA DE VALIDADE"] as number)
              ? dayjs(
                  getDateFromLocaleString(
                    bulkKit["DATA DE VALIDADE"].toString()
                  )
                )
                  .add(3, "hour")
                  .toISOString()
              : dayjs(
                  getFixedDateFromExcel(bulkKit["DATA DE VALIDADE"] as number)
                )
                  .add(3, "hour")
                  .toISOString()
            : null;
          return {
            ...
            idsMetodologiasPagamento: [
              "661ec619e03128a48f94b4db",
              "661ec635e03128a48f94b4dc",
              "661ec65ae03128a48f94b4dd",
            ],
  ...
  async function handleBulkOperation(
    kits: (TKit & { _id: string | null; excluir: boolean })[]
  ) {
    if (kits.length == 0) return toast.error("Nenhum kit foi encontrado.");

    try {
      const createArr = kits
        .filter((k) => !k._id)
        .map((k) => ({ ...k, excluir: undefined })) as TKit[];
      const updateArr = kits
        .filter((k) => !!k._id && !k.excluir)
        .map((k) => ({ ...k, excluir: undefined })) as TKitDTO[];
      const deleteArr = kits
        .filter((k) => !!k._id && !!k.excluir)
        .map((k) => ({ ...k, excluir: undefined })) as TKitDTO[];
      ...
      if (deleteArr.length > 0)
        await deleteManyKits({ ids: deleteArr.map((k) => k._id) });
  ...
  const creatingCount =
    kitsHolder?.reduce((acc, current) => (!current._id ? acc + 1 : acc), 0) ||
    0;
  const updateCount =
    kitsHolder?.reduce(
      (acc, current) => (!!current._id && !current.excluir ? acc + 1 : acc),
      0
    ) || 0;
  const deleteCount =
    kitsHolder?.reduce(
          ...
          <div className="flex grow flex-col gap-y-2 overflow-y-auto overscroll-y-auto px-2 py-1 scrollbar-thin scrollbar-track-gray-100 scrollbar-thumb-gray-300">
            ...
            {kitsHolder.length > 0 ? (
              <div className="flex w-full flex-col">
                <h1 className="rounded bg-gray-800 p-1 text-center font-bold text-white">
                  KITS IDENTIFICADOS
                </h1>
                <p className="text-sm font-medium text-gray-500">
                  Foram identificados{" "}
                  <strong className="text-[#f25041]">{creatingCount}</strong>{" "}
                  kits para criação.
                </p>
                <p className="text-sm font-medium text-gray-500">
                  Foram identificados{" "}
                  <strong className="text-[#f25041]">{updateCount}</strong> kits
                  para atualização.
                </p>
                <p className="text-sm font-medium text-gray-500">
                  Foram identificados{" "}
                  <strong className="text-[#f25041]">{deleteCount}</strong> kits
                  para exclusão.
                </p>
                <div className="mt-2 flex w-full flex-col gap-1">
                  <h1 className="text-start font-Inter text-xs font-medium leading-none tracking-tight">
                      ...
                      <button
                        onClick={() =>
                          setKitsHolder((prev) =>
                            prev.map((k) => ({
                              ...k,
                              produtos: [...k.produtos, ...type.produtos],
                            }))
                          )
                        }
                        key={index}
                      ...
                      <button
                        onClick={() =>
                          setKitsHolder((prev) =>
                            prev.map((k) => ({
                              ...k,
                              servicos: [...k.servicos, ...type.servicos],
                            }))
                          )
                        }
                        key={index}
