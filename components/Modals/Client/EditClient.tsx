...
function EditClient({
	...
	const { mutate: handleUpdateClient, isPending } = useMutationWithFeedback({
		...
		callbackFn: async () => {
			...
			if (additionalAffectedQuery)
				await queryClient.invalidateQueries({
					queryKey: additionalAffectedQuery,
				});
		},
	});
		...
		>
			<div className="fixed left-[50%] top-[50%] z-100 h-[80%] w-[93%] translate-x-[-50%] translate-y-[-50%] rounded-md bg-background p-[10px]">
				<div className="flex h-full flex-col">
					...
					{isError ? (
						<ErrorComponent msg="Erro ao buscar informações do cliente." />
					) : null}
					{isSuccess ? (
					...
					<div className="my-2 flex w-full items-center justify-end px-4">
						<button
							...
							// @ts-ignore
							onClick={() =>
								handleUpdateClient({ id: clientId, changes: clientInfo })
							}
