...
function RevenueFileAttachmentMenu({ revenueId, projectId, clientId, opportunityId, analysisId, homologationId, purchaseId, session }: RevenueFileAttachmentMenuProps) {
	...
	async function handleFileReferenceCreation() {
		try {
			...
			const vinculationId = attachmentHolder.idRevenue || attachmentHolder.idProject || attachmentHolder.idOpportunity || attachmentHolder.idClient || "naodefinido";
							...
							<div className="mt-4 flex w-full flex-col flex-wrap items-center justify-center gap-4 lg:flex-row">
								<div className="w-fit">
									<CheckboxInput
										...
										editable={false}
										handleChange={(value) => setAttachmentHolder((prev) => ({ ...prev, idRevenue: revenueId }))}
									/>
								</div>
								<div className="w-fit">
									<CheckboxInput
