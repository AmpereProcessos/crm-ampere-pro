...
type RevenueCategoriesMenuProps = {
	infoHolder: TRevenue;
	setInfoHolder: React.Dispatch<React.SetStateAction<TRevenue>>;
};
function RevenueCategoriesMenu({ infoHolder, setInfoHolder }: RevenueCategoriesMenuProps) {
	...
	return (
		<div className="flex w-full flex-col gap-y-2">
			<h1 className="w-full bg-gray-500 p-1 text-center text-xs font-medium text-white">CATEGORIAS</h1>
			<div className="my-2 flex w-full flex-wrap items-center gap-2">
				{infoHolder.categorias.length > 0 ? (
					infoHolder.categorias.map((category, index) => (
						<div className="group flex items-center gap-2 rounded border border-blue-800 px-4 py-2 shadow-md">
							...
							<button onClick={() => removeCategory(index)} className="text-xs text-red-600 opacity-0 duration-300 ease-in-out group-hover:opacity-100">
								<VscChromeClose />
							</button>
						</div>
					))
				) : (
					<p className="flex w-full grow items-center justify-center py-2 text-center text-sm font-medium italic tracking-tight text-gray-500">Sem categorias adicionadas.</p>
				)}
			</div>
			<div className="flex w-full flex-col gap-1">
				<div className="flex w-full items-end gap-2">
					...
					<div className="w-fit">
						<button onClick={() => addCategory(categoryHolder)} className="rounded-md bg-black p-3 text-sm font-medium text-white duration-300 ease-in-out hover:bg-gray-700">
							ADICIONAR
						</button>
					</div>
				</div>
...
export default RevenueCategoriesMenu;
