import { ProcessAutomationConditionTypesOptions, ProcessAutomationEntitiesSpecs, TProcessAutomationEntities, TProcessAutomationEntitySpec } from "@/utils/process-settings";
...
import { getProcessAutomationConditionOptions, processAutomationConditionAlias, TProcessAutomationConditionData } from "@/utils/process-settings/helpers";
	...
	return (
		<div className="flex w-[80%] flex-col gap-2 self-center rounded-md border border-gray-500 p-2 font-Inter">
			...
			<div className="flex w-full flex-col items-center justify-center">
				...
				<h1 className="w-fit rounded border border-cyan-500 p-2 text-center text-xs font-bold text-cyan-500">{activeAutomationReference.entityLabel}</h1>
			</div>
				...
				<div className="flex w-full flex-col gap-2 lg:flex-row">
					<div className="w-full lg:w-1/2">
						<NumberInput
							...
							value={infoHolder.gatilho.entre?.minimo != null && infoHolder.gatilho.entre?.minimo != undefined ? Number(infoHolder.gatilho.entre?.minimo) : null}
							handleChange={(value) =>
								setInfoHolder((prev) => ({
									...prev,
					...
					<div className="w-full lg:w-1/2">
						<NumberInput
							...
							value={infoHolder.gatilho.entre?.maximo != null && infoHolder.gatilho.entre?.maximo != undefined ? Number(infoHolder.gatilho.entre?.maximo) : null}
							handleChange={(value) =>
								setInfoHolder((prev) => ({
									...prev,
			...
			{infoHolder.retorno.entidade == "Revenue" ? (
				<h1 className="w-full rounded border border-orange-500 bg-orange-50 p-1 text-center text-xs tracking-tight text-orange-500 ">
					POR PADRÃO, OS DADOS A SEREM UTILIZADOS NA RECEITA TERÃO COMO BASE O VALOR DE VENDA DO PROJETO E O FRACIONAMENTO DE RECEBIMENTOS DO MÉTODO DE PAGAMENTO.
				</h1>
			) : null}
		</div>
	);
