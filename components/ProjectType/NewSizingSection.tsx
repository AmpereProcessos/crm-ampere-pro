...
function NewSizingSection({ infoHolder, setInfoHolder, addSection, closeMenu }: NewSizingSectionProps) {
	...
	return (
		<div className="flex w-[80%] flex-col self-center rounded-md border border-gray-500 p-2 font-Inter">
			...
			<p className="my-2 text-center text-sm tracking-tight text-gray-500">Escolha um nome para uma seção de campos de preenchimento e escolha os campos que comporão essa seção.</p>
			<input
			...
			<div className="my-2 flex flex-wrap items-center gap-2">
				{AutomaticPremissesBySaleCategory[infoHolder.categoriaVenda].length > 0 ? (
					...
					<p className="flex w-full grow items-center justify-center py-2 text-center font-medium italic tracking-tight text-gray-500">Sem campos automáticos.</p>
				)}
			</div>
			...
			<div className="my-2 flex items-center justify-end gap-2">
				<button onClick={() => addSection(sizingHolder)} className="rounded bg-blue-600 p-1 px-4 text-xs font-medium text-white duration-300 ease-in-out hover:bg-blue-800">
					CADASTRAR SEÇÃO
				</button>
			</div>
		</div>
