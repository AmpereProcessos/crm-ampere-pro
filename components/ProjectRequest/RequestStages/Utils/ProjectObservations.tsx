	...
	return (
		<div className="flex w-full flex-col gap-1">
			...
			<p className="my-1 w-full text-center text-sm font-light tracking-tighter text-gray-700">
				Se houver alguma informação relevante em relação ao serviços a serem prestado, a detalhes dos produtos, da negociação, da suprimentação (compra/entrega), da execução do
				projeto, etc, escolha o tópico aplicavél e preencha a observação e clique em ADICIONAR.
			</p>
			<div className="my-1 flex w-full flex-wrap items-center justify-center gap-4 px-2">
				{ProjectObservationTopics.map((topic, index) => (
					<button
						...
						className={`rounded-lg ${observationHolder.assunto == topic ? "bg-blue-500 text-white" : "bg-transparent text-blue-500"} border border-blue-500 px-2 py-1 text-xs font-bold`}
					>
						{topic}
					</button>
				))}
			...
			<div className="mt-1 flex w-full items-center justify-end">
				<button onClick={() => addObservation(observationHolder)} className="rounded bg-black px-4 py-1 text-sm font-medium text-white duration-300 ease-in-out hover:bg-gray-700">
					ADICIONAR
				</button>
			</div>
