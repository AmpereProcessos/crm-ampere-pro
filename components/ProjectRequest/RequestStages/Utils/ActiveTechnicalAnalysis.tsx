	...
	return (
		<div className="relative flex w-full items-center rounded-md border border-gray-300">
			...
			<div className="flex grow flex-col p-3">
				<div className="flex w-full grow flex-col gap-1">
					<div className="flex w-full flex-col items-start justify-between gap-1 lg:flex-row">
						<div className="flex grow flex-col items-center lg:items-start">
							<h1 className="w-full text-center text-sm font-bold leading-none tracking-tight duration-300 lg:text-start">{analysis.tipoSolicitacao || "NÃO DEFINIDO"}</h1>
				...
				<h1 className="w-full rounded-md bg-gray-100 p-2 py-1 text-center text-xs font-medium text-gray-500">{analysis.conclusao.observacoes || "SEM OBSERVAÇÃO PARA A ANÁLISE"}</h1>
				<div className="mt-2 flex w-full flex-wrap items-center justify-around gap-2">
					<div className={`flex w-full items-center justify-center gap-1 rounded p-1 lg:w-fit ${analysis.conclusao.espaco ? "bg-green-50 text-green-500" : "bg-red-50 text-red-500"}`}>
						<p className="text-center text-xs font-medium tracking-tight">{analysis.conclusao.espaco ? "POSSUI ESPAÇO PARA INSTALAÇÃO" : "NÃO POSSUI ESPAÇO PARA INSTALAÇÃO"}</p>
					</div>
					<div
						className={`flex w-full items-center justify-center gap-1 rounded p-1 lg:w-fit ${!analysis.conclusao.inclinacao ? "bg-green-50 text-green-500" : "bg-red-50 text-red-500"}`}
					>
					...
					<div
						className={`flex w-full items-center justify-center gap-1 rounded p-1 lg:w-fit ${!analysis.conclusao.sombreamento ? "bg-green-50 text-green-500" : "bg-red-50 text-red-500"}`}
					>
						<p className="text-center text-xs font-medium tracking-tight">{!analysis.conclusao.sombreamento ? "NÃO SOFRERÁ COM SOMBREAMENTO" : "SOFRERÁ COM SOMBREAMENTO"}</p>
					</div>
				</div>
					...
					{analysis.custos.map((cost, index) => (
						<div key={index} className="flex items-center gap-2 rounded border border-gray-500 p-2">
							...
							{userHasPricingViewPermission ? (
								<h1 className="min-w-fit rounded-full bg-gray-800 px-2 py-1 text-[0.58rem] font-medium text-white lg:text-xs">{formatToMoney(cost.total || 0)}</h1>
							) : null}
						</div>
					))}
				</div>
				<div className="mt-2 flex w-full flex-col items-center justify-end gap-2 lg:flex-row">
					{analysis.dataEfetivacao ? (
						<>
							...
							<div className="flex items-center gap-1">
								<Avatar fallback={formatNameAsInitials(analysis.analista?.nome || "R")} url={analysis.analista?.avatar_url || undefined} height={20} width={20} />
