...
function HomologationBlock({ infoHolder, setInfoHolder, documentationHolder, setDocumentationHolder, moveToNextStage, moveToPreviousStage, session }: HomologationBlockProps) {
		...
		<div className="flex w-full grow flex-col gap-2">
						...
						{selectableHomologations ? (
							selectableHomologations.length > 0 ? (
								selectableHomologations.map((homologation) => (
									<SelectableHomologation key={homologation._id} homologation={homologation} selectHomologation={(id) => setInfoHolder((prev) => ({ ...prev, idHomologacao: id }))} />
								))
							) : (
			...
			<div className="flex w-full items-center justify-between">
				...
				{activeHomologation ? (
					...
					<button onClick={() => validateAndProceed()} className="rounded bg-orange-700 px-4 py-1 text-sm font-medium text-white duration-300 ease-in-out hover:bg-orange-800">
