...
function ClientBlock({ infoHolder, setInfoHolder, moveToNextStage, moveToPreviousStage, clientId, session }: ClientBlockProps) {
	...
	async function validateAndProceed() {
		...
		if (!clientInfo.numeroOuIdentificador || clientInfo.numeroOuIdentificador.trim().length <= 1) return toast.error("Preencha um número/identificador de residência válido.");
