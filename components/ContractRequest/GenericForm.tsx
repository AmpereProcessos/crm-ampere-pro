...
function GenericForm({ requestInfo, setRequestInfo, proposal }: GenericFormProps) {
	...
	async function handleRequestContract({
		...
		files,
		proposal,
	}: { request: TContractRequest; files: { [key: string]: File | string | null }; proposal: TProposalDTOWithOpportunity }) {
		try {
	...
	return (
		<>
			...
			{stage === 4 ? (
				<DeliveryInfo requestInfo={requestInfo} setRequestInfo={setRequestInfo} showActions={true} goToNextStage={() => setStage(5)} goToPreviousStage={() => setStage(3)} />
			) : null}
			{stage === 5 ? (
				<PaymentInfo requestInfo={requestInfo} setRequestInfo={setRequestInfo} showActions={true} goToNextStage={() => setStage(6)} goToPreviousStage={() => setStage(4)} />
			) : null}
			{stage === 6 ? (
				<HomologationInfo requestInfo={requestInfo} setRequestInfo={setRequestInfo} showActions={true} goToNextStage={() => setStage(7)} goToPreviousStage={() => setStage(5)} />
			) : null}
			{stage === 7 ? (
				<OtherServices requestInfo={requestInfo} setRequestInfo={setRequestInfo} showActions={true} goToNextStage={() => setStage(8)} goToPreviousStage={() => setStage(6)} />
			) : null}
			{stage === 8 ? (
				<Documents
