...
function HomologationInfo({
  ...
  const [homologationModality, setHomologationModality] = useState<"CONVENCIONAL" | "FAST TRACK">(
    requestInfo.homologacaoFastTrack ? "FAST TRACK" : "CONVENCIONAL",
  );
        ...
        <div className="flex w-full items-center justify-center">
          <div className="w-fit">
            <CheckboxInput
              ...
              handleChange={(value) =>
                setRequestInfo((prev) => ({ ...prev, realizarHomologacao: value }))
              }
            />
          </div>
        </div>
          ...
          {requestInfo.realizarHomologacao ? (
            <>
              <div className="w-full flex flex-col gap-2">
                ...
                <div className="w-full rounded-sm border border-blue-800 bg-blue-50 p-2">
                  <div className="flex w-full items-center justify-between gap-2">
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        homologationModality === "CONVENCIONAL"
                          ? "bg-blue-800 text-xs text-primary-foreground"
                          : "",
                      )}
                    />
                  </div>
                  <p className="text-sm tracking-tight text-blue-700">
                    Modalidade convencional no qual a potência de homologação será a máxima possível
                    liberada pela concessionária local.
                  </p>
                  <p className="text-sm tracking-tight text-blue-700">
                    Em casos em que a potência de contrato ultrapasse a de homologação, o
                    prosseguimento se dará com o consentimento do cliente sobre possíveis riscos.
                  </p>
                </div>
                {requestInfo.potPico && requestInfo.potPico <= 7.5 ? (
                  <div className="w-full rounded-sm border border-green-800 bg-green-50 p-2">
                    <div className="flex w-full items-center justify-between gap-2">
                      ...
                      <button
                        ...
                        className={cn(
                          ...
                          homologationModality === "FAST TRACK"
                    ...
                    <p className="text-sm tracking-tight text-green-700">
                      Nova modalidade liberada pela ANEEL que dispensa análise de potência para
                      sistemas de até 7,5KW.
                    </p>
                    <p className="text-sm tracking-tight text-green-700">
                      <strong>IMPORTANTE:</strong> para participar dessa modalidade a instalação
                      deve optar pela abdicação permanente de distribuição de créditos.
                    </p>
                  </div>
                ) : null}
              </div>
              <div className="flex w-full items-center justify-end">
                <button
                  type="button"
                  onClick={() => useInfoInfoFromContract()}
              ...
              <div className="w-full flex flex-col gap-4">
                <div className="flex items-center gap-2 bg-primary/20 px-2 py-1 rounded-sm w-fit">
                  <ChevronRight size={15} />
                  <h1 className="text-xs tracking-tight font-medium text-start w-fit">
                    INFORMAÇÕES DO TITULAR DA INSTALAÇÃO
                  </h1>
                </div>
                <div className="w-full flex items-center gap-2 flex-col lg:flex-row">
                  ...
                  <div className="w-full lg:w-1/3">
                    <TextInput
                      ...
                      value={
                        requestInfo.cpf_cnpjTitularProjeto ? requestInfo.cpf_cnpjTitularProjeto : ""
                      }
                      editable={true}
                      handleChange={(value) =>
                        setRequestInfo({
                ...
                <div className="flex items-center gap-2 bg-primary/20 px-2 py-1 rounded-sm w-fit">
                  <ChevronRight size={15} />
                  <h1 className="text-xs tracking-tight font-medium text-start w-fit">
                    CREDENCIAIS DO APP DA CONCESSIONÁRIA
                  </h1>
                </div>
                <div className="w-full flex items-center gap-2 flex-col lg:flex-row">
              ...
              <div className="w-full flex flex-col gap-4">
                  ...
                  <div className="w-fit">
                    <CheckboxInput
                      ...
                      handleChange={(value) =>
                        setRequestInfo((prev) => ({
                          ...prev,
                          possuiDistribuicao: value ? "SIM" : "NÃO",
                          distribuições: !value ? [] : prev.distribuicoes,
                        }))
                      }
                    />
                  </div>
                ...
                {requestInfo.possuiDistribuicao === "SIM" ? (
                  <CreditDistributions
                    distributions={requestInfo.distribuicoes}
...
function CreditDistributions({
  distributions,
  addDistribution,
  removeDistribution,
}: CreditDistributionsProps) {
  const [distributionHolder, setDistributionHolder] = useState<
    TContractRequest["distribuicoes"][number]
  >({
    numInstalacao: "",
    excedente: 0,
  });
    ...
    <>
      ...
      {distributions.length > 0 ? (
        distributions.map((distribution, index) => (
          <div
            key={`distribuicao-${distribution.numInstalacao}-${index}`}
            className="flex w-full justify-between items-center gap-1 rounded-sm border border-primary bg-background p-2"
          >
        ...
        <div className="w-full text-center text-sm font-medium tracking-tight text-primary/80">
          Nenhuma distribuição adicionada.
        </div>
      )}
    </>
  );
