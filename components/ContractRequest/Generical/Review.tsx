...
function Review({ proposal, requestInfo, setRequestInfo, documentsFile, setDocumentsFile, handleRequestContract }: ReviewProps) {
	...
	if (isSuccess) {
		return (
			<div className="flex w-full grow flex-col items-center justify-center gap-2 text-green-500">
				...
				<p className="text-lg font-medium tracking-tight text-gray-500">Contrato solicitado com sucesso !</p>
			</div>
		);
	}
	if (isError) {
	...
	return (
		<div className="flex w-full flex-col bg-[#fff] pb-2 gap-6 grow">
			<span className="px-4 py-2 bg-[#fead41] rounded-lg font-bold tracking-tight text-black self-center w-fit text-center text-sm">REVISÃO DAS INFORMAÇÕES</span>
			<ContractInfo requestInfo={requestInfo} setRequestInfo={setRequestInfo} showActions={false} goToNextStage={() => {}} />
			<ProductsAndServices editable={true} requestInfo={requestInfo} setRequestInfo={setRequestInfo} showActions={false} goToNextStage={() => {}} goToPreviousStage={() => {}} />
			<SignaturePlans proposal={proposal} requestInfo={requestInfo} setRequestInfo={setRequestInfo} showActions={false} goToNextStage={() => {}} goToPreviousStage={() => {}} />
