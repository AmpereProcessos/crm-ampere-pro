...
function OemItemsMenu({ requestInfo, setRequestInfo }: OemItemsMenuProps) {
	function getInitialItems(request: TContractRequest) {
		...
		for (let i = 0; i < maxModulesPropertyLength; i++) {
			...
			const iterModuleDescription = modulesDescription[i];
			modulesArr.push({
				tipo: "MÓDULO" as const,
				descricao: iterModuleDescription,
				qtde: Number(iterModuleQty),
				potencia: Number(iterModulePower),
			});
		}
		for (let i = 0; i < maxInvertersPropertyLength; i++) {
			const iterInverterQty = invertersQty[i];
			const iterInverterPower = invertersPower[i];
			const iterInverterDescription = invertersDescription[i];
			invertersArr.push({
				tipo: "INVERSOR" as const,
				descricao: iterInverterDescription,
				qtde: Number(iterInverterQty),
				potencia: Number(iterInverterPower),
			});
		}

		const itemsArr = [...modulesArr, ...invertersArr];
