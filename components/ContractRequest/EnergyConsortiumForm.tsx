...
import ContractInfo from './EnergyConsortium/ContractInfo';
import DocumentAttachment from './EnergyConsortium/DocumentAttachment';
import PaymentInfo from './EnergyConsortium/PaymentInfo';

type EnergyConsortiumFormProps = {
  requestInfo: TContractRequest;
  setRequestInfo: React.Dispatch<React.SetStateAction<TContractRequest>>;
  proposeInfo: TProposalDTOWithOpportunity;
  closeForm: () => void;
};
function EnergyConsortiumForm({ requestInfo, setRequestInfo, proposeInfo, closeForm }: EnergyConsortiumFormProps) {
  ...
  return (
    <>
      {stage == 1 ? (
        <ContractInfo requestInfo={requestInfo} setRequestInfo={setRequestInfo} goToNextStage={() => setStage((prev) => prev + 1)} />
      ) : null}
      {stage == 2 ? (
        <PaymentInfo
...
export default EnergyConsortiumForm;
