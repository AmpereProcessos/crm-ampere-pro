	...
	return (
		<div className="flex w-full flex-col gap-1">
			...
			<div className="flex items-center justify-end">
				<button className="rounded bg-black p-1 px-4 text-sm font-medium text-white duration-300 ease-in-out hover:bg-gray-700" onClick={() => addItem(descriptiveHolder)}>
					ADICIONAR ITEM
				</button>
			</div>
				...
				<div className="flex w-full flex-col items-center gap-2">
					{infoHolder.descritivo.length > 0 ? (
						infoHolder.descritivo.map((product, index) => (
							<div onClick={() => removeItem(index)} className="flex w-fit cursor-pointer items-center justify-center gap-2 rounded-md bg-transparent px-2 py-1 hover:bg-red-100">
