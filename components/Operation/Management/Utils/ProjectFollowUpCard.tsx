...
function ProjectFollowUpCard({ project }: ProjectFollowUpCardProps) {
	...
	return (
		<div className="flex w-full flex-col gap-2 rounded-sm border border-primary/50 bg-background p-4 shadow-md">
				...
				<div className="flex items-center gap-1">
					...
					{project.idProjetoCRM ? (
						<Link href={`/comercial/oportunidades/id/${project.idProjetoCRM}`}>
							<h1 className="text-xs font-medium tracking-tight text-primary/70 duration-300 ease-in-out hover:text-cyan-500">{project.identificador}</h1>
						</Link>
					) : (
