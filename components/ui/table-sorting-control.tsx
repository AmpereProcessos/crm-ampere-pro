...
export function TableSortingControl<T>({
  ...
  return (
    <div className={cn('mt-4 bg-linear-to-r from-gray-50 to-primary/10 border border-primary/30 rounded-lg shadow-md', currentSize.container)}>
      <div className='flex items-center justify-between flex-wrap gap-4'>
        <div className='flex items-center gap-3'>
          <div className='flex items-center gap-2'>
            <div className='w-2 h-2 rounded-full' style={{ backgroundColor: accentColor }} />
            <span className={cn('font-semibold text-primary/70 uppercase tracking-wider', currentSize.title)}>{title}</span>
          </div>
        </div>

        <div className='flex items-center gap-4 flex-wrap'>
          <div className='flex items-center gap-2'>
            <span className={cn('font-medium text-primary/60 uppercase tracking-wide', currentSize.label)}>Ordenar por:</span>
            <Select value={sortConfig.field as string} onValueChange={(value) => handleFieldChange(value as T)}>
              <SelectTrigger
                className={cn(
                  'border-primary/30 bg-background hover:border-[${accentColor}] focus:border-[${accentColor}] focus:ring-1 transition-colors',
                  currentSize.select
                )}
                style={
                  {
              ...
              <SelectContent className='border-primary/30 shadow-lg'>
          ...
          <div className='flex items-center gap-2'>
            <span className={cn('font-medium text-primary/60 uppercase tracking-wide', currentSize.label)}>Direção:</span>
            <Button
