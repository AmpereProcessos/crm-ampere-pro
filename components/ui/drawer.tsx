	...
	({ className, children, ...props }, ref) => (
		<DrawerPortal>
			...
			<DrawerPrimitive.Content
				ref={ref}
				className={cn("fixed inset-x-0 bottom-0 z-[100] mt-24 flex h-auto flex-col rounded-t-[10px] border border-gray-300 bg-white dark:border-gray-800 dark:bg-gray-950", className)}
				{...props}
			>
...
const DrawerDescription = React.forwardRef<React.ElementRef<typeof DrawerPrimitive.Description>, React.ComponentPropsWithoutRef<typeof DrawerPrimitive.Description>>(
	({ className, ...props }, ref) => <DrawerPrimitive.Description ref={ref} className={cn("text-sm text-gray-500 dark:text-gray-400", className)} {...props} />,
);
