	...
	({ active, payload, className, indicator = "dot", hideLabel = false, hideIndicator = false, label, labelFormatter, labelClassName, formatter, color, nameKey, labelKey }, ref) => {
		...
		return (
			<div
				ref={ref}
				className={cn(
					"grid min-w-[8rem] items-start gap-1.5 rounded-lg border border-gray-300 border-gray-300/50 bg-white px-2.5 py-1.5 text-xs shadow-xl dark:border-gray-800 dark:border-gray-800/50 dark:bg-gray-950",
					className,
				)}
			>
				...
				<div className="grid gap-1.5">
					{payload.map((item, index) => {
						...
						return (
							<div
								key={item.dataKey}
								className={cn("flex w-full items-stretch gap-2 [&>svg]:h-2.5 [&>svg]:w-2.5 [&>svg]:text-gray-500 dark:[&>svg]:text-gray-400", indicator === "dot" && "items-center")}
							>
								{formatter && item?.value !== undefined && item.name ? (
									...
									<>
										...
										<div className={cn("flex flex-1 gap-2 justify-between leading-none", nestLabel ? "items-end" : "items-center")}>
											...
											{item.value && <span className="font-mono font-medium tabular-nums text-gray-950 dark:text-gray-50">{item.value.toLocaleString()}</span>}
										</div>
									</>
								)}
							</div>
