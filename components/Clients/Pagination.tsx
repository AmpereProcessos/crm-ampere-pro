		...
		<button
			...
			className={cn(
				...
				"max-w-7 min-w-7 min-h-7 h-7 max-h-7 w-7",
				{
					...
					"border-transparent text-black hover:bg-gray-500": activePage !== p,
				},
			)}
			// className={`${
	...
	return (
		<div className="my-2 flex w-full flex-col items-center gap-1 ">
			{totalPages > 1 ? (
				<>
					<p className="w-full text-center text-sm leading-none tracking-tight text-gray-500">
						Um número grande de clientes foi encontrado, separamos em páginas
						para facilitar a visualização. Clique na página desejada para
						visualizar os demais clientes.
					</p>
					<div className="flex items-center justify-center gap-4">
						<button
							...
							className="flex select-none items-center gap-2 rounded-full px-2 lg:px-6 py-3 text-center align-middle font-sans text-xs font-bold uppercase text-gray-900 transition-all disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none hover:bg-gray-900/10 active:bg-gray-900/20"
							type="button"
						>
							<svg
			...
			<p className="w-full text-center text-sm leading-none tracking-tight text-gray-500">
				{clientsMatched
					? clientsMatched > 0
						? `${clientsMatched} clientes encontrados.`
						: `${clientsMatched} cliente encontrado.`
					: "..."}
			</p>
			<p className="w-full text-center text-sm leading-none tracking-tight text-gray-500">
				{clientsShowing
					? clientsShowing > 0
						? `Mostrando ${clientsShowing} clientes.`
						: `Mostrando ${clientsShowing} cliente.`
					: "..."}
			</p>
		</div>
	);
