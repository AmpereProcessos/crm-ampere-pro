  ...
  return (
    <div className='flex w-full flex-col gap-2'>
      ...
      <div className='flex w-full flex-col'>
        ...
        <p className='w-full text-center text-xs leading-none tracking-tight'>
          Os parâmetros de ativação da próxima etapa serão baseados nesse processo.
        </p>
      </div>
      <div className='flex flex-wrap items-center gap-2'>
        {entityReference.returnableEntities.map((p, index) => (
          <button
            ...
            className={`grow rounded-sm border border-green-700 p-1  text-xs font-medium text-green-500  duration-300 ease-in-out hover:bg-green-700  hover:text-primary-foreground`}
          >
            {getEntityLabel(p)}
          </button>
        ))}
