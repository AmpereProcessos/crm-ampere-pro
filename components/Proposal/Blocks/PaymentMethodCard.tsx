		...
		<div className="flex w-full flex-col rounded-md border border-primary/30 p-2">
					...
					<button
						onClick={() => {
							if (isSelected) removeMethod(methodHolder.id);
								...
								selectMethod({
									id: methodHolder.id,
									nome: methodHolder.nome,
									descricao: methodHolder.descricao,
									fracionamento: methodHolder.fracionamento,
								});
						}}
			...
			<div className="flex w-full flex-wrap items-center justify-start gap-2">
				{methodHolder.fracionamento.map((fractionnement, itemIndex) => (
					<div
						key={itemIndex}
						className={`flex w-full flex-col rounded-md border border-primary/50 p-2 shadow-md lg:w-[${fractionnementWidth ? fractionnementWidth : "450px"}]`}
					>
						...
						<div className="mt-2 flex w-full flex-wrap items-center justify-between">
							...
							{fractionnement.taxaJuros ? (
								<div className="flex items-center gap-2">
									<FaPercentage />
									<p className="text-[0.65rem] font-medium leading-none tracking-tight text-primary/70 lg:text-xs">{fractionnement.taxaJuros} DE JUROS</p>
								</div>
							) : null}
							{fractionnement.taxaUnica ? (
								<div className="flex items-center gap-2">
									<FaPercentage />
									<p className="text-[0.65rem] font-medium leading-none tracking-tight text-primary/70 lg:text-xs">{fractionnement.taxaUnica} DE USO</p>
								</div>
							) : null}
						</div>
					</div>
			...
			<div className="mt-2 flex w-full items-center justify-end">
				<h1 className="text-sm text-primary/70">
					VALOR FINAL DE: <strong className="text-primary">{formatToMoney(getPaymentMethodFinalValue({ method: methodHolder, proposalValue }))}</strong>
				</h1>
			</div>
		</div>
	);
