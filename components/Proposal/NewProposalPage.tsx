		...
		return (
			<div className="flex h-full flex-col md:flex-row">
				...
				<div className="flex w-full max-w-full grow flex-col overflow-x-hidden bg-[#f8f9fa]">
					<div className="flex h-fit w-full flex-col items-center gap-2 bg-black px-2 py-4 lg:flex-row">
						<div className="flex w-full items-center justify-center lg:w-1/3">
							<Link href={`/comercial/oportunidades/id/${opportunityId}`}>
								<button type="button" className="flex items-center gap-1 rounded bg-white px-2 py-1 text-xs text-black">
									<FaStepBackward />
					...
					{saleCategory === "KIT" ? (
						<ProposalWithKits opportunity={opportunity} partner={opportunity.parceiro} session={session} infoHolder={infoHolder} setInfoHolder={setInfoHolder} />
					) : null}
					{saleCategory === "PLANO" ? (
						<ProposalWithPlans opportunity={opportunity} partner={opportunity.parceiro} session={session} infoHolder={infoHolder} setInfoHolder={setInfoHolder} />
					) : null}
					{saleCategory === "PRODUTOS" ? (
						<ProposalWithProducts opportunity={opportunity} partner={opportunity.parceiro} session={session} infoHolder={infoHolder} setInfoHolder={setInfoHolder} />
					) : null}
					{saleCategory === "SERVIÇOS" ? (
						<ProposalWithServices opportunity={opportunity} partner={opportunity.parceiro} session={session} infoHolder={infoHolder} setInfoHolder={setInfoHolder} />
					) : null}
				</div>
			</div>
		);
