					...
					<div className="flex w-full flex-col items-center justify-between gap-4 border-b border-gray-200 pb-2">
						<div className="flex w-full flex-col items-center justify-center gap-2 lg:flex-row lg:justify-between">
							...
							<WinBlock
								...
								handleWin={() => {
									if (proposal.oportunidadeDados.categoriaVenda === "PLANO" && proposal.planos.length > 1) return toast.error("Defina o plano a ser vendido para prosseguir com o ganho.");
									...
									{Object.entries(proposal.premissas)
										...
										.map(([key, value], index) => (
											<div key={`${key}`} className="flex w-full flex-col items-center justify-between gap-1 lg:flex-row">
												<h1 className="text-xs font-medium uppercase leading-none tracking-tight text-gray-500">{formatProposalPremissesLabel(key as keyof TProposalPremisses)}</h1>
												<h1 className="text-end text-sm font-medium leading-none tracking-tight lg:text-xs">{formatProposalPremissesValue({ key: key as keyof TProposalPremisses, value })}</h1>
											</div>
										))}
								</div>
							</div>
