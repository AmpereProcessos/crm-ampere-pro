		...
		<div ref={proposalDocumentRef} className="relative flex h-fit w-full flex-col overflow-hidden bg-white lg:h-[297mm] lg:w-[210mm]">
			...
			<div className="flex w-full flex-col">
				...
				<div className="flex w-full items-center gap-1 p-1">
					...
					<h1 className="w-1/4 text-center text-[0.7rem] font-bold text-red-500">{formatToMoney(getScenariosInfo({ proposal, opportunity }).monthlyEnergyExpense)}</h1>
					<h1 className="w-1/4 text-center text-[0.7rem] font-bold text-red-500">{formatToMoney(getScenariosInfo({ proposal, opportunity }).annualEnergyExpense)}</h1>
					<h1 className="w-1/4 text-center text-[0.7rem] font-bold text-red-500">{formatToMoney(getScenariosInfo({ proposal, opportunity }).twentyFiveYearsEnergyExpense)}</h1>
				</div>
			</div>
			<div className="mt-2 flex w-full flex-col">
				...
				<div className="flex w-full items-center gap-1 p-1">
					...
					<h1 className="w-1/4 text-center text-[0.7rem] font-bold text-green-500">{formatToMoney(getScenariosInfo({ proposal, opportunity }).monthlySavedValue)}</h1>
					<h1 className="w-1/4 text-center text-[0.7rem] font-bold text-green-500">{formatToMoney(getScenariosInfo({ proposal, opportunity }).annualSavedValue)}</h1>
					<h1 className="w-1/4 text-center text-[0.7rem] font-bold text-green-500">{formatToMoney(getScenariosInfo({ proposal, opportunity }).twentyFiveYearsSavedValue)}</h1>
				</div>
			</div>
			...
			<div className="mt-2 flex w-full flex-col gap-1">
				...
				{proposal.servicos.map((service) => (
					<div key={`${service.descricao}-${service.garantia}`} className="flex w-full items-center gap-1">
						...
						<h1 className="w-1/4 text-center text-[0.7rem] font-bold text-black">{service.garantia > 1 ? `${service.garantia} ANOS` : `${service.garantia} ANO`}</h1>
					</div>
				))}
			</div>
			<div className="mt-2 flex w-full flex-col gap-1">
				...
				<div className="flex w-full flex-col gap-1">
					{proposal.pagamento.metodos.map((method, index) => (
						<div key={`${method.descricao}-${index}`} className="flex w-full flex-col border border-gray-500 p-2">
									...
									{method.fracionamento.map((fractionnement, itemIndex) => (
										<div key={`${method.descricao}-${itemIndex}`} className={"flex w-fit min-w-fit items-center gap-1 rounded-md border border-gray-200 p-2 shadow-sm"}>
											...
											<h1 className="text-[0.55rem] font-medium leading-none tracking-tight">
												...
												<strong>{formatToMoney(getFractionnementValue({ fractionnement, proposalValue: proposal.valor }) / (fractionnement.parcelas || fractionnement.maximoParcelas))}</strong>
											</h1>
										</div>
									))}
								</div>
			...
			<span className="px-2 text-center text-[0.47rem] font-medium">
				A GERAÇÃO PREVISTA DE ENERGIA MENSAL PODE VARIAR DE ACORDO COM CONDIÇÕES CLIMÁTICAS E TÉCNICAS DO LOCAL DE INSTALAÇÃO DO PROJETO; OS VALORES AQUI PROPOSTOS SÃO APENAS UMA
				PREVISÃO DE GERAÇÃO DE ACORDO COM FATORES GENÉRICOS DA REGIÃO CONSIDERADA
			</span>
