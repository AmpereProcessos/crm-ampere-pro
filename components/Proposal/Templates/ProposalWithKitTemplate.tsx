...
function ProposalWithKitTemplate({ proposalDocumentRef, proposal, opportunity, partner }: ProposalWithKitTemplateProps) {
	return (
		<div ref={proposalDocumentRef} className="relative flex h-fit w-full flex-col overflow-hidden bg-white lg:h-[297mm] lg:w-[210mm]">
			<div className="flex h-fit w-full items-center justify-between rounded-bl-md rounded-br-md bg-black p-4">
				...
				<div className="flex flex-col items-end">{partner.logo_url ? <Image src={partner.logo_url} width={60} height={60} alt="WHITE LOGO" quality={100} /> : null}</div>
			</div>
			<div className="flex w-full grow flex-col">
				...
				<div className="flex w-full flex-col gap-1">
					{proposal.produtos.map((product, index) => (
						<div key={`${product.categoria}-${product.fabricante}-${product.modelo}`} className="flex w-full flex-col border border-gray-500 p-2">
							<div className="flex w-full flex-col items-start justify-between gap-2">
								<div className="flex w-full items-center justify-between">
									<div className="flex items-center gap-1">
										<div className="flex h-[35px] w-[35px] items-center justify-center rounded-full border border-black p-1 text-[15px]">{renderCategoryIcon(product.categoria, 18)}</div>
								...
								<div className="flex w-full items-center justify-end gap-2 pl-2">
									...
									{product.garantia ? (
										<div className="flex items-center gap-1">
											...
											<p className="whitespace-nowrap text-[0.6rem] font-light text-gray-500 lg:text-xs">{product.garantia > 1 ? `${product.garantia} ANOS` : `${product.garantia} ANO`}</p>
										</div>
									) : null}
								</div>
							</div>
				...
				<div className="flex w-full flex-col gap-1">
					{proposal.servicos.map((service, index) => (
						<div key={`${service.descricao}-${index}`} className="flex w-full flex-col border border-gray-500 p-2">
							<div className="flex w-full flex-col items-start justify-between gap-2">
								<div className="flex w-full items-center justify-between">
									<div className="flex items-center gap-1">
								...
								{service.observacoes.trim().length > 0 ? <p className="w-full text-start text-[0.65rem] text-gray-500">{service.observacoes || "..."}</p> : null}

								<div className="flex w-full items-center justify-end gap-2 pl-2">
									{service.garantia ? (
										<div className="flex items-center gap-1 whitespace-nowrap">
											<AiOutlineSafety size={12} />
											<p className="whitespace-nowrap text-[0.6rem] font-light text-gray-500 lg:text-xs">{service.garantia > 1 ? `${service.garantia} ANOS` : `${service.garantia} ANO`}</p>
										</div>
									) : null}
								</div>
							</div>
