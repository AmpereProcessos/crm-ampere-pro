import { formatFormulaItem, operators, renderConditionPhrase, variablesAlias } from '@/utils/pricing/helpers';
...
function NewPricingUnit({
  ...
  function addResultFormula() {
    if (resultHolder.condicao.aplicavel) {
      ...
      if (resultHolder.condicao.tipo == 'IGUAL_TEXTO' && !resultHolder.condicao.igual)
    ...
    const newResultFormulas = currentResultFormulas.sort((a, b) =>
  ...
  return (
    <div className='flex w-[80%] flex-col self-center rounded-md border border-primary/50 p-2 font-Inter'>
      ...
      <div className='flex w-full flex-wrap items-center justify-around gap-2'>
        {operators.map((op, index) => (
          <button
            ...
            className='grow rounded-sm border border-primary/30 bg-primary/10 p-2 text-center font-medium text-primary/70 duration-300 ease-in-out hover:scale-105 hover:bg-primary/30'
          >
            {op}
          </button>
        ))}
        <button
          ...
          className='flex grow items-center justify-center rounded-sm border border-red-300 bg-red-100 p-2 text-center font-medium duration-300 ease-in-out hover:scale-105 hover:bg-red-200'
        >
          <FiDelete size={23} />
        </button>
      </div>
          ...
          <div key={index} className='mb-1 flex w-full flex-col items-center gap-2 rounded-md border border-[#A0E9FF] p-1 md:flex-row'>
            ...
            <div className='flex items-center gap-1'>
              <button
                ...
                className='flex items-center justify-center rounded-lg p-1 text-primary/90 duration-300 ease-linear hover:scale-105 hover:bg-primary/30'
              >
                <MdContentCopy />
              </button>
              <button
