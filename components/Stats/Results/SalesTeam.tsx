...
import { TableSortingControl, type SortConfig } from "@/components/ui/table-sorting-control";
	...
	return (
		<div className="flex w-full flex-col">
			...
			<div className="mt-2 flex w-full flex-col items-start gap-2 lg:flex-row">
				<StatCard icon={<AiOutlineThunderbolt />} label="Potência Vendida" promoters={promoters || []} statKey="potenciaPico" stats={stats} statsLoading={isLoading} />
			...
			<div className="mt-2 flex w-full flex-col items-start gap-2 lg:flex-row">
				<ConversionStatCard numeratorStatKey="projetosVendidos" denominatorStatKey="projetosCriados" promoters={promoters || []} stats={stats} statsLoading={isLoading} />
			</div>
		</div>
	);
}
...
function SalesTeamOverallResults({ stats, promoters }: { stats: TSellerSalesResults; promoters: TUserDTOWithSaleGoals[] }) {
	...
	return (
		<div className="mt-2 flex h-[400px] max-h-[600px] w-full flex-col rounded-xl border border-gray-300 bg-[#fff] p-6 shadow-sm lg:h-[600px]">
			...
			<div className="overscroll-y mt-2 gap-1.5 flex w-full grow flex-col overflow-y-auto px-2 scrollbar-thin scrollbar-track-gray-100 scrollbar-thumb-gray-300">
				...
				{statsAsList.map((stat) => (
					<div key={stat.nome} className="w-full flex flex-col">
						<div className="hidden lg:flex items-center justify-between gap-2 border border-gray-300 rounded-md p-2">
							<div className="w-1/6 flex items-center gap-2">
						...
						<div className="flex lg:hidden items-center justify-between gap-2 border border-gray-300 rounded-md p-2 flex-col">
