...
type PaymentsPaginationMenuProps = {
	activePage: number;
	selectPage: (x: number) => void;
	totalPages: number;
	paymentsMatched?: number;
	paymentsShowing?: number;
	queryLoading: boolean;
};
function PaymentsPaginationMenu({ activePage, selectPage, totalPages, paymentsMatched, paymentsShowing, queryLoading }: PaymentsPaginationMenuProps) {
	return (
			...
			{totalPages > 1 ? (
				<>
					<p className="w-full text-center text-sm leading-none tracking-tight text-primary/70">
						Um número grande de pagamentos foi encontrado, separamos em páginas para facilitar a visualização. Clique na página desejada para visualizar os
						demais pagamentos.
					</p>
...
export default PaymentsPaginationMenu;
