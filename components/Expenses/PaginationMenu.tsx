...
type ExpensePaginationMenuProps = {
	activePage: number;
	selectPage: (x: number) => void;
	totalPages: number;
	expensesMatched?: number;
	expensesShowing?: number;
	queryLoading: boolean;
};
function ExpensePaginationMenu({ activePage, selectPage, totalPages, expensesMatched, expensesShowing, queryLoading }: ExpensePaginationMenuProps) {
	return (
			...
			{totalPages > 1 ? (
				<>
					<p className="w-full text-center text-sm leading-none tracking-tight text-primary/70">
						Um número grande de despesas foi encontrado, separamos em páginas para facilitar a visualização. Clique na página desejada para visualizar os demais
						despesas.
					</p>
...
export default ExpensePaginationMenu;
