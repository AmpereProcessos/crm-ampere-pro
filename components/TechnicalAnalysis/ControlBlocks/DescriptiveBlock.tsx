		...
		<div className="mt-4 flex w-full flex-col">
			...
			<div className="mt-2 flex w-full flex-col gap-2">
				...
				<div className="mb-2 flex w-full items-center justify-end">
					<button onClick={addDescriptiveItem} className="rounded border border-green-500 p-1 font-bold text-green-500 duration-300 ease-in-out hover:bg-green-500 hover:text-white">
						ADICIONAR ITEM
					</button>
				</div>
			</div>
			{infoHolder.descritivo?.length > 0 ? (
				infoHolder.descritivo.map((item, index) => (
					<div key={index} className="mb-1 flex w-full flex-col rounded border border-gray-300 p-2 shadow-md">
				...
				<p className="flex w-full grow items-center justify-center py-2 text-center font-medium italic tracking-tight text-gray-500">Nenhum item adicionado à lista...</p>
			)}
		</div>
	);
}
