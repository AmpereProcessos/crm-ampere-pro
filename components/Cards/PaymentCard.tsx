...
function getPaymentStatusTag({ paymentDate, isPaid }: { paymentDate: string | null; isPaid: boolean }) {
	...
	if (!paymentDate) return <div className="rounded-md bg-yellow-600 px-2 py-0.5 text-xxs font-medium text-primary-foreground lg:text-[0.6rem]">A PAGAR</div>;
	const isOverDue = dayjs(new Date()).isAfter(paymentDate);
	console.log(paymentDate, isOverDue);
	if (!isOverDue) return <div className="rounded-md bg-yellow-600 px-2 py-0.5 text-xxs font-medium text-primary-foreground lg:text-[0.6rem]">A PAGAR</div>;
