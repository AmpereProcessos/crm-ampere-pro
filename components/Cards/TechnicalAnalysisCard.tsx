...
function TechnicalAnalysisCard({ analysis, handleClick, userHasEditPermission }: TechnicalAnalysisCardProps) {
	return (
		<div key={analysis._id} className="flex min-h-[180px] w-full gap-2 rounded-md border border-gray-500 bg-[#fff] shadow-md lg:w-[500px]">
				...
				<div className="flex w-full grow flex-col">
					...
					<div className="mt-2 flex w-full items-center justify-between">
						<div className="flex items-center gap-2">
							<BsCode size={18} />
							<p className="text-[0.65rem] font-medium leading-none tracking-tight text-gray-500 lg:text-xs">{analysis.oportunidade.nome || "OPORTUNIDADE NÃO VINCULADA"}</p>
						</div>
						<div className="flex items-end gap-2">
							<TbWorld size={18} />
							<p className="text-[0.65rem] font-medium leading-none tracking-tight text-gray-500 lg:text-xs">{analysis.complexidade || "COMPLEXIDADE NÃO DEFINIDA"}</p>
						</div>
					</div>
					<div className="mt-2 flex w-full items-center justify-between">
						<div className="flex items-center gap-2">
							<Avatar url={analysis.requerente.avatar_url || undefined} fallback={formatNameAsInitials(analysis.requerente.nome || "U")} width={20} height={20} />
						...
						{analysis.dataEfetivacao ? (
							<>
								...
								<div className="flex items-center gap-1">
									...
									<p className="text-[0.55rem] text-gray-500">{formatDecimalPlaces(getHoursDiff({ start: analysis.dataInsercao, finish: analysis.dataEfetivacao }), 0, 2)}h</p>
								</div>
							</>
						) : null}
					</div>
