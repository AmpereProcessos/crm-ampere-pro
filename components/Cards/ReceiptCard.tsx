...
function getPaymentStatusTag({ paymentDate, isPaid }: { paymentDate: string | null; isPaid: boolean }) {
	if (!!isPaid) return <div className="rounded-md bg-green-600 px-2 py-0.5 text-xxs font-medium text-primary-foreground lg:text-[0.6rem]">RECEBIDO</div>;
	if (!paymentDate) return <div className="rounded-md bg-yellow-600 px-2 py-0.5 text-xxs font-medium text-primary-foreground lg:text-[0.6rem]">A RECEBER</div>;
	const isOverDue = dayjs(new Date()).isAfter(paymentDate);
	if (!isOverDue) return <div className="rounded-md bg-yellow-600 px-2 py-0.5 text-xxs font-medium text-primary-foreground lg:text-[0.6rem]">A RECEBER</div>;
			...
			<div className="flex flex-col rounded-sm border border-primary/50 p-2">
				<div className="flex w-full flex-col items-center justify-between gap-2 lg:flex-row">
					<div className="flex w-full items-center justify-start gap-2 lg:w-fit">
						...
						<h1 className="text-[0.65rem] font-bold tracking-tight text-primary/80 lg:text-xs">{receipt.titulo}</h1>
