...
function getStatusTag(status?: TPPSCallDTO["status"] | null) {
	if (status == "REALIZADO") return <h1 className="w-fit self-center rounded border border-green-500 p-1 text-center text-[0.6rem] font-black text-green-500">REALIZADO</h1>;
	if (status == "EM ANDAMENTO") return <h1 className="w-fit self-center rounded border border-blue-500 p-1 text-center text-[0.6rem] font-black text-blue-500">EM ANDAMENTO</h1>;
	...
	if (status == "PENDENTE") return <h1 className="w-fit self-center rounded border border-red-400 p-1 text-center text-[0.6rem] font-black text-red-400">PENDENTE</h1>;
							...
							{call.projeto.id ? (
								<div className="flex w-full items-center gap-1">
									...
									<Link href={`/comercial/oportunidades/id/${call.projeto.id}`}>
										<p className="text-[0.65rem] font-medium uppercase tracking-tight text-gray-500 duration-300 ease-in-out hover:text-cyan-500">{call.projeto.nome}</p>
									</Link>
								</div>
							) : null}
						</div>
