...
function getStatusTag(status?: string | null) {
	...
	if (status == "PENDÊNCIA EXTERNA") return <h1 className={`w-fit self-center rounded border border-gray-800 p-1 text-center text-[0.6rem] font-black text-gray-800`}>{status}</h1>;

	if (status == "CONCLUÍDA") return <h1 className={`w-fit self-center rounded border border-green-500 p-1 text-center text-[0.6rem] font-black text-green-500`}>{status}</h1>;
...
function ProjectPurchaseCard({ purchase }: ProjectPurchaseCardProps) {
	...
	return (
		<div className="flex w-full flex-col gap-2 rounded border border-gray-300 bg-[#fff] p-3">
			...
			<div className="flex w-full flex-col items-center justify-between gap-2 lg:flex-row">
				...
				<div className="flex flex-col items-center gap-1 lg:items-end">
					...
					{renderDeliveryText(purchase.entrega).dates ? <p className="text-[0.6rem] font-medium leading-none tracking-tight">{renderDeliveryText(purchase.entrega).dates}</p> : null}
