...
function ProposalTechnicalAnalysis({ isSelected, analysis, vinculateAnalysis, unvinculateAnalysis, userHasPricingViewPermission }: ProposalTechnicalAnalysisProps) {
	return (
		<div className="flex w-full gap-2 rounded-md border border-gray-500 bg-[#fff] font-Inter shadow-md">
			<div className="flex grow flex-col gap-2 p-4">
				...
				<div className="flex w-full flex-wrap items-center justify-around gap-2">
					<div className={`flex items-center gap-1 rounded p-1 ${analysis.conclusao.espaco ? "bg-green-50 text-green-500" : "bg-red-50 text-red-500"}`}>
						<p className="text-xs font-medium tracking-tight">{analysis.conclusao.espaco ? "POSSUI ESPAÇO PARA INSTALAÇÃO" : "NÃO POSSUI ESPAÇO PARA INSTALAÇÃO"}</p>
					</div>
					<div className={`flex items-center gap-1 rounded p-1 ${!analysis.conclusao.inclinacao ? "bg-green-50 text-green-500" : "bg-red-50 text-red-500"}`}>
						<p className="text-xs font-medium tracking-tight">{!analysis.conclusao.inclinacao ? "NÃO NECESSÁRIO ESTRUTURA DE INCLINAÇÃO" : "NECESSÁRIO ESTRUTURA DE INCLINAÇÃO"}</p>
					</div>
					<div className={`flex items-center gap-1 rounded p-1 ${!analysis.conclusao.sombreamento ? "bg-green-50 text-green-500" : "bg-red-50 text-red-500"}`}>
						<p className="text-xs font-medium tracking-tight">{!analysis.conclusao.sombreamento ? "NÃO SOFRERÁ COM SOMBREAMENTO" : "SOFRERÁ COM SOMBREAMENTO"}</p>
					</div>
				</div>
