...
function Product({ product, handleClick, userHasEditPermission, userHasPricingViewPermission }: ProductProps) {
	return (
		<div className="flex w-full gap-2 rounded-md border border-gray-500 bg-[#fff] font-Inter shadow-md lg:w-[450px]">
				...
				<div className="flex w-full items-center justify-between gap-2">
					<div className="flex items-center gap-1 text-black ">
						...
						{userHasEditPermission ? (
							<h1 onClick={() => handleClick(product._id)} className="cursor-pointer text-sm font-black leading-none tracking-tight duration-300 ease-in-out hover:text-cyan-500">
								{product.modelo}
							</h1>
						) : (
