...
import { BsCalendarCheck, BsCalendarPlus, BsCheck2, BsCheck2All, BsCode } from "react-icons/bs";
...
function NotificationCard({ notification, session }: NotificationCardProps) {
	...
	async function updateNotification({ notification, read, user }: { notification: TNotificationDTO; read: boolean; user: TUserSession["user"] }) {
	...
	return (
		<div className={`flex w-full flex-col gap-2 rounded-sm border ${userReadingInformation ? "border-green-600" : "border-primary/50"} p-3`}>
			...
			<div className="flex w-full grow flex-col">
				<h1 className="rounded-md border border-primary/30 px-2 py-1 text-[0.65rem] font-medium tracking-tight">{notification.mensagem}</h1>
			</div>
        ...
        <div className="flex w-[80%] flex-col">
          <div className="flex w-full items-center gap-1 text-xs italic text-primary/80">
      ...
      <div className="flex w-full items-center justify-end pt-1">
        <button
          onClick={() => setAsRead()}
          disabled={!!isRead}
          className={`${isRead ? 'text-green-500' : 'text-primary/60 hover:text-blue-500'}  duration-300 hover:scale-110 `}
        >
          <BsCheckAll
            style={{
              fontSize: '20px',
