...
function getStatusColor(status: string) {
	...
	if (status == "PENDENTE") return <h1 className="w-fit self-center rounded border border-gray-500 p-1 text-center text-[0.6rem] font-black text-gray-500">PENDENTE</h1>;
				...
				<div className="mt-2 flex w-full flex-col items-center justify-between gap-2 lg:flex-row">
					<div className="flex items-center gap-2">
						{analysis.dataEfetivacao || analysis.status == "CONCLUIDO" ? (
							<button className="rounded bg-cyan-500 px-2 py-1 text-center text-[0.6rem] font-bold text-white" onClick={() => setReportsMenuIsOpen((prev) => !prev)}>
								LAUDOS
							</button>
						) : null}
