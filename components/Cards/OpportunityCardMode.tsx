...
function OpportunityCardMode({ opportunity }: OpportunitiesCardModeProps) {
  ...
  return (
    <div className='relative flex min-h-[110px] w-full flex-col justify-between rounded-sm border border-primary/40 bg-background shadow-md'>
              ...
              if (index <= 1)
                return (
                  <div className='flex items-center gap-1'>
                    ...
                    <p className='text-[0.65rem] font-light text-primary/40'>{resp.nome}</p>
                  </div>
                );
              else return null;
            })}
            {opportunity.responsaveis.length > 2 ? <p className='text-[0.65rem] font-light text-primary/40'>...</p> : null}
          </div>
