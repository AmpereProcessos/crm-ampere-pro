...
function getStatusTag(status?: TPPSCallDTO["status"] | null) {
	if (status == "REALIZADO")
		return <h1 className="w-fit self-center rounded-sm border border-green-500 p-1 text-center text-[0.6rem] font-black text-green-500">REALIZADO</h1>;
	if (status == "EM ANDAMENTO")
		return <h1 className="w-fit self-center rounded-sm border border-blue-500 p-1 text-center text-[0.6rem] font-black text-blue-500">EM ANDAMENTO</h1>;
	if (status == "AGUARDANDO VENDEDOR")
		return <h1 className="w-fit self-center rounded-sm border border-orange-500 p-1 text-center text-[0.6rem] font-black text-orange-500">AG. VENDENDOR</h1>;
	if (status == "PENDENTE")
		return <h1 className="w-fit self-center rounded-sm border border-red-400 p-1 text-center text-[0.6rem] font-black text-red-400">PENDENTE</h1>;
...
function PPSCallCard({ call, session }: PPSCallCardProps) {
	return (
		<div className="flex w-full gap-2 rounded-md border border-primary/30 shadow-md">
