...
function ProposalSignaturePlan({ index, selectedIds, plan, proposal, opportunity, handleSelect, handleRemove, userHasPricingView }: ProposalSignaturePlanProps) {
	...
	return (
		<div key={plan._id} className="flex min-h-[250px] w-full gap-2 rounded-md border border-gray-300 shadow-md lg:w-[800px]">
                ...
                Sem produtos vinculados ao plano
						...
						<div className="flex w-full flex-wrap items-center gap-2">
							{plan.servicos.map((service, index) => (
								<div key={index} className="mt-1 flex flex-col gap-1 rounded-md border border-gray-300 p-2">
									<div className="flex w-full items-center justify-between gap-2">
										<div className="flex items-center gap-1">
					...
					<div className="mt-2 flex w-full items-center justify-end gap-2">
						...
						{!selectedIds.includes(plan._id) ? (
							...
							<button onClick={() => handleRemove(index)} className="rounded-full border border-red-500 p-1 text-red-500 duration-300 ease-in-out hover:bg-red-500 hover:text-white">
								<MdOutlineRemove />
							</button>
						)}
					</div>
