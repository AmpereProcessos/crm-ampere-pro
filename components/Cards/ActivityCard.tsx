...
function ActivityCard({ activity }: ActivityCardProps) {
	...
	return (
		<div className="flex  w-full max-w-full  gap-2 rounded-md border border-primary/30 shadow-md">
			<div
				className={`flex h-full w-[5px] rounded-bl-md rounded-tl-md ${getBarColor(info.dataVencimento || undefined)}`}
			/>
			<div className="flex w-full grow flex-col gap-1 p-3 pl-1">
				<div className="flex w-full grow flex-col">
					<div className="flex items-center gap-2">
						<h1 className="w-full text-start text-xs font-bold leading-none tracking-tight">
					...
					<h1 className="text-xs leading-none tracking-tight text-primary/70">
						RESPONSÁVEIS
					</h1>
					<div className="flex grow flex-wrap items-center gap-2">
						{activity.responsaveis.map((resp) => (
							...
							>
								...
								/>
								<p className="text-[0.65rem] font-medium tracking-tight text-primary/70">
									{resp.nome}
								</p>
							</div>
						))}
				...
				<div className="flex w-full items-center justify-end">
					{renderDateDiffText(info.dataVencimento || undefined)}
				</div>
			</div>
		</div>
	);
