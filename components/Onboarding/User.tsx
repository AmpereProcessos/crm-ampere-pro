...
import { GeneralUserSchema, type TAuthor, type TUser } from '@/utils/schemas/user.schema';
...
function User({ partnerId, setAuthorHolder, goToNextStage, goToPreviousStage }: UserProps) {
  ...
  async function handleUserCreation() {
    try {
      GeneralUserSchema.parse(userHolder);
                ...
                <div className='relative flex h-[200px] w-[200px]  items-center justify-center'>
                  {avatarFile && !!editImage.temporaryUrl ? (
                    ...
                    <div className=' flex h-[120px] w-[120px] items-center justify-center rounded-full border border-primary/30 bg-primary/30'>
                      <p className='absolute w-full text-center text-xxs font-bold text-primary/70'>ESCOLHA SEU AVATAR</p>
                    </div>
                  )}
                  <input
                    onChange={(e) => {
