...
import {
  TOpportunitiesByFastSearchParams,
  useOpportunitiesBySearch,
} from "@/utils/queries/opportunities";
  ...
  return (
    <div className="relative flex items-center justify-center">
      <Button
        ...
        className={cn(`w-fit rounded-md p-3 border border-[#fead41] bg-[#fead41] text-black`)}
      >
        <AiOutlineSearch className="w-4 h-4 min-w-4 min-h-4" />
      </Button>
...
function FilterMenu({ closeMenu }: FilterMenuProps) {
  const [queryParams, setSearchParams] = useState<TOpportunitiesByFastSearchParams>({
    searchParam: "",
    page: 1,
  });
  ...
  const {
    ...
    isFetching,
    error,
  } = useOpportunitiesBySearch(finalQueryParams);
      ...
      <SheetContent className="sm:max-w-1/2 w-full lg:w-1/2">
        <div className="flex h-full w-full flex-col">
          <SheetHeader>
            ...
            <SheetDescription>
              Escolha aqui parâmetros para filtrar as oportunidades.
            </SheetDescription>
          </SheetHeader>
          <TextInput
...
function OpportunityCard({ opportunity }: { opportunity: TOpportunitySimplifiedDTO }) {
  ...
  return (
    <div
      key={opportunity._id}
      className="flex w-full flex-col gap-1 rounded-sm border border-primary bg-background p-2 shadow-md"
    >
      <div className="flex w-full flex-col items-center justify-between gap-2 lg:flex-row">
        <div className="flex flex-wrap items-center gap-2">
          <p className="text-sm font-bold leading-none tracking-tight text-[#fead41]">
            {opportunity.identificador}
          </p>
        ...
        <div className="flex w-full flex-wrap items-center justify-center gap-2 lg:grow lg:justify-start">
          <div className="flex items-center gap-1">
            ...
            <h1 className="py-0.5 text-center text-[0.6rem] font-medium italic text-primary/80">
              {opportunity.tipo.titulo}
            </h1>
          </div>
          <div className="flex items-center gap-1">
            <FaPhone size={12} />
            <h1 className="py-0.5 text-center text-[0.6rem] font-medium italic text-primary/80">
              {opportunity.cliente?.telefonePrimario}
            </h1>
          </div>
          <div className="flex items-center gap-1">
            <FaUser size={12} />
            ...
            <div className="flex -space-x-1 overflow-hidden">
              {opportunity.responsaveis.map((responsible) => (
                <Avatar
                  ...
                  width={20}
                  height={20}
                  fallback={formatNameAsInitials(responsible.nome)}
                />
              ))}
            </div>
          </div>
