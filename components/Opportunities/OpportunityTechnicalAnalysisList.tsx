	...
	return (
		<div className="flex h-[320px] w-full flex-col rounded-md border border-primary/30 bg-background p-3 shadow-lg lg:h-[230px] lg:w-[60%]">
			<div className="flex  h-[40px] items-center  justify-between border-b border-primary/30 pb-2">
				...
				<button onClick={() => setRequestModalIsOpen(true)} className="hidden rounded-sm bg-green-600 p-1 text-sm font-bold text-primary-foreground lg:flex">
					SOLICITAR ANÁLISE
				</button>
				<button onClick={() => setRequestModalIsOpen(true)} className="flex rounded-sm bg-green-600 p-1 text-sm font-bold text-primary-foreground lg:hidden">
					<MdAdd />
				</button>
			</div>
			<div className="overscroll-y relative flex w-full grow flex-col gap-2 overflow-y-auto py-1 pr-2 scrollbar-thin scrollbar-track-primary/10 scrollbar-thumb-primary/30">
				...
				{successTechAnalysis && technicalAnalysis ? (
					technicalAnalysis?.length > 0 ? (
						...
						<p className="flex grow items-center justify-center italic text-primary/70">Não foram encontradas análises técnicas vinculadas a esse projeto...</p>
					)
				) : null}
			</div>
