...
function ActivityCard({ event, opportunityId }: ActivityCardProps) {
	...
	return (
		<div className="flex w-full flex-col gap-2 rounded-md border border-primary/30 p-2">
			...
			{event.observacoes ? (
				<div className="flex w-full items-center">
					<p className="w-full rounded-sm border border-primary/30 bg-primary/10 p-3 text-center font-Inter text-xs text-primary/70">{event.observacoes}</p>
				</div>
			) : null}
			<div className="flex w-full items-center justify-between">
				<div className="flex items-center gap-2">
				...
				<div className="flex items-center gap-2">
					...
					<p className="text-xs font-medium text-primary/70">{event.dataInsercao ? dayjs(event.dataInsercao).format("DD/MM/YYYY HH:mm") : null}</p>{" "}
				</div>
			</div>
		</div>
	);
