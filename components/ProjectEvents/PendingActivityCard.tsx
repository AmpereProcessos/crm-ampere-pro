...
function PendingActivityCard({ activity }: PendingActivityCardProps) {
	...
	return (
		<div className="flex w-full max-w-full gap-2 rounded-md border border-primary/30 shadow-md">
			<div
				className={`flex h-full w-[5px] rounded-tl-md rounded-bl-md ${getBarColor(info.dataVencimento || undefined)}`}
			/>
			<div className="flex w-full grow flex-col gap-1 p-3 pl-1">
				<div className="flex w-full grow flex-col">
					...
					{info.oportunidade.id ? (
						<Link href={`/comercial/oportunidades/id/${info.oportunidade.id}`}>
							<div className="flex items-center gap-1">
								...
								<p className="mt-1 font-bold text-primary/70 text-xs hover:text-cyan-500">
					...
					<h1 className="my-2 w-full rounded-md bg-primary/10 p-2 py-1 text-center font-medium text-primary/70 text-xs">
						{activity.descricao}
					</h1>
					<h1 className="text-primary/70 text-xs leading-none tracking-tight">
						RESPONSÁVEIS
					</h1>
						...
						{activity.responsaveis.map((resp) => (
							...
							>
								<Avatar
									...
									width={15}
								/>
								<p className="font-medium text-[0.65rem] text-primary/70 tracking-tight">
									{resp.nome}
								</p>
							</div>
						))}
