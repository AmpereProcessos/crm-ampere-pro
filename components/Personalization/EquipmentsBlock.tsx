			...
			<div className="flex max-h-[600px] w-full grow flex-wrap items-start justify-around gap-2 overflow-y-auto overscroll-y-auto p-2 scrollbar-thin scrollbar-track-gray-100 scrollbar-thumb-gray-300">
				...
				{isSuccess ? (
					equipments.length > 0 ? (
						...
						<p className="flex w-full grow items-center justify-center py-2 text-center font-medium italic tracking-tight text-gray-500">Nenhum equipamento encontrado.</p>
					)
				) : null}
			</div>
			<div className="flex w-full items-center justify-end p-2">
				{newEquipmentMenuIsOpen ? (
					<button className="rounded bg-red-500 p-1 px-4 text-sm font-medium text-white duration-300 ease-in-out hover:bg-red-600" onClick={() => setNewEquipmentMenuIsOpen(false)}>
						FECHAR MENU
					</button>
				) : (
					<button className="rounded bg-green-500 p-1 px-4 text-sm font-medium text-white duration-300 ease-in-out hover:bg-green-600" onClick={() => setNewEquipmentMenuIsOpen(true)}>
						NOVO EQUIPAMENTO
					</button>
				)}
			</div>
