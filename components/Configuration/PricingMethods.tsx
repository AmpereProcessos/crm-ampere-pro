...
const FixedPricingMethod = [
  "660dab0b0fcb72da4ed8c35e",
  "660de08225fee32a2237fa37",
  "661400485ce24a96d0c62c30",
  "661455bf6eaecfde21b552a7",
];
...
function PricingMethods({ session }: PricingMethodsProps) {
  ...
  return (
    <div className="flex h-full grow flex-col">
      <div className="flex w-full flex-col items-center justify-between border-b border-primary/30 pb-2 lg:flex-row">
        <div className="flex flex-col">
          <h1 className={`text-lg font-bold uppercase`}>
            Controle de metodologias de precificação
          </h1>
          <p className="text-sm text-[#71717A]">
            Gerencie, adicione e edite os metodologias de precificação
          </p>
        </div>
        <Button
      ...
      {newPricingMethodModalIsOpen ? (
        <NewPricingMethod
          session={session}
          closeModal={() => setNewPricingMethodModalIsOpen(false)}
        />
      ) : null}
                ...
                <div className="flex w-full flex-col gap-2">
                  ...
                  <div className="flex w-full flex-wrap items-center justify-start gap-2">
                    {method.itens.map((item, itemIndex) => (
                      <div
                        key={itemIndex}
                        className="rounded-lg border border-primary/30 bg-primary/20 px-2 py-1 text-[0.57rem] font-medium"
                      >
                        {item.nome}
                      </div>
                    ))}
                    ...
                    <>
                      <div className={`flex items-center gap-2`}>
                        <div className="ites-center flex gap-1">
                          ...
                          <p className={`text-xs font-medium text-primary/70`}>
                            {formatDateAsLocale(method.dataInsercao, true)}
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center justify-center gap-1">
                        <Avatar
                          ...
                          height={20}
                          width={20}
                          url={method.autor?.avatar_url || undefined}
                        />
