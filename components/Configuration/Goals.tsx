...
function Goals({ session }: GoalsProps) {
  ...
  return (
    <div className="flex h-full grow flex-col">
      <div className="flex w-full items-center justify-between border-gray-300 border-b pb-2">
        <div className="flex w-full items-center justify-between border-gray-300 border-b pb-2">
          ...
          <button
            className="h-9 whitespace-nowrap rounded bg-gray-900 px-4 py-2 font-medium text-sm text-white shadow enabled:hover:bg-gray-800 enabled:hover:text-white disabled:bg-gray-500 disabled:text-white"
...
function GoalCard({ goal, handleEditClick }: { goal: Exclude<TGetGoalsRouteOutput['data']['default'], undefined>[number]; handleEditClick: () => void }) {
    ...
    <div className="flex w-full flex-col gap-3 rounded-md border border-primary/30 p-2">
      ...
      <div className="flex w-full flex-col gap-2">
        ...
        <div className="flex w-full flex-wrap items-center justify-start gap-2">
          ...
          <GoalValueCard icon={<BadgeDollarSign className="h-3.5 w-3.5" />} label="VALOR VENDIDO (R$)" value={formatToMoney(goal.objetivo.valorVendido)} />
          <GoalValueCard
            icon={<Zap className="h-3.5 w-3.5" />}
            label="POTÊNCIA VENDIDA (kWp)"
            value={`${formatDecimalPlaces(goal.objetivo.potenciaVendida)}kWp`}
          />
          <GoalValueCard icon={<Percent className="h-3.5 w-3.5" />} label="CONVERSÃO EM ENVIO (%)" value={goal.objetivo.oportunidadesEnviadasConversao} />
          <GoalValueCard icon={<Percent className="h-3.5 w-3.5" />} label="CONVERSÃO EM GANHO (%)" value={goal.objetivo.oportunidadesGanhasConversao} />
        </div>
      </div>
    </div>
  );
