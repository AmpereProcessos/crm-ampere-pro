...
function SelectInput<T>({ value, options, handleChange, selectedItemLabel, showLabel = true, label, labelClassName, width }: SelectInputProps<T>) {
        ...
        return (
            <Popover open={menuIsOpen} onOpenChange={setMenuIsOpen}>
                <div className={cn('w-full flex flex-col gap-1', `lg:w-[${width ? width : '350px'}]`)}>
                    {showLabel ? (
                        <label className={cn('font-sans font-bold  text-[#353432] text-start', labelClassName)}>
                            {label}
                        </label>
                    ) : null}
