...
function LeadReceiversBlock() {
  ...
  return (
    <div className='flex w-full flex-col gap-2'>
      <h1 className='text-xs font-medium leading-none tracking-tight text-primary/50'>RECEBEDORES DE LEAD</h1>
          ...
          leadReceivers.length > 0 ? (
            leadReceivers.map((receiver) => (
              <div key={receiver._id} className='flex items-center gap-2 rounded-lg border border-cyan-500 p-1 px-2 shadow-md'>
                ...
                <p className='text-xs font-medium tracking-tight text-primary/50'>{receiver.nome}</p>
              </div>
            ))
          ) : (
            <h1 className='text-center text-xs font-medium leading-none tracking-tight text-primary/50'>Não há recebedores de leads configurados.</h1>
          )
        ) : null}
      </div>
    </div>
