...
function DropdownSelect<T extends {}>({
  ...
  return (
    <div ref={ref} className={`relative flex  items-center ${width ? `w-full lg:w-[${width}]` : 'w-full lg:w-[400px]'}`}>
      <div className='flex min-h-[46.6px] w-full grow items-center justify-between rounded-md border border-primary/30 bg-background p-2 shadow-md'>
        {selectMenuIsOpen ? (
          <input
            type='text'
            autoFocus
      ...
      {selectMenuIsOpen ? (
        <div className='absolute top-[55px] z-100 flex h-[250px] w-full flex-col self-center rounded-md border border-primary/30 bg-background p-2 shadow-md'>
          ...
          <div className='overscroll-y my-2 flex w-full grow flex-col gap-1 overflow-y-auto scrollbar-thin scrollbar-track-primary/10 scrollbar-thumb-primary/30'>
            <div
              onClick={() => resetState()}
              className={`flex w-full cursor-pointer items-center rounded-sm p-1 px-2 hover:bg-primary/10 ${!(selectedId != undefined) ? 'bg-primary/10' : ''}`}
            >
            ...
            {items ? (
              items.map((item, index) => (
                <div
                  ...
                  className={`flex w-full cursor-pointer items-center rounded-sm p-1 px-2 hover:bg-primary/10 ${selectedId == item.id ? 'bg-primary/10' : ''}`}
                >
                  <p className='grow font-medium text-primary'>{item.label}</p>
