			...
			<div ref={ref} draggable={false} className={`relative flex w-full flex-col gap-1 lg:w-[${width ? width : "350px"}]`}>
				...
				{selectMenuIsOpen ? (
					<div
						...
						} z-[100] flex h-[250px] max-h-[250px] w-full flex-col self-center overflow-y-auto overscroll-y-auto rounded-md border border-primary/20 bg-[#fff] p-2 py-1 shadow-md scrollbar-thin scrollbar-track-gray-100 scrollbar-thumb-gray-300`}
					>
						<button
							type="button"
							onClick={() => resetState()}
						...
						>
							<p className="grow text-sm font-medium text-primary text-start">{resetOptionLabel}</p>
						...
						{items ? (
							items.map((item, index) => (
								<button
									...
									className={`flex w-full cursor-pointer items-center rounded p-1 px-2 hover:bg-primary/20 ${selectedIds?.includes(item.id) ? "bg-primary/20" : ""}`}
								>
									<p className="grow text-sm font-medium text-primary text-start">{item.label}</p>
