				...
				<div className="flex -space-x-1">
					{selectedOptions.slice(0, displayCount).map((item, index) => (
						<div key={item.id} className="flex items-center gap-1 border-2 border-white bg-primary/20 rounded-lg p-0.5 px-1">
							...
							<p className="text-xxs font-medium text-primary">{item.label}</p>
						</div>
					))}
				</div>
				{remainingCount > 0 && (
					<div className="ml-1 flex h-6 min-w-6 items-center justify-center rounded-full bg-primary/10 px-2 text-xs font-medium text-primary">
						+{remainingCount}
					</div>
				)}
			</div>
			...
			<div ref={ref} className={`relative flex w-full flex-col gap-1 lg:w-[${width ? width : "350px"}]`}>
				...
				{selectMenuIsOpen ? (
					<div
						...
						} z-100 flex h-[250px] max-h-[250px] w-full flex-col gap-1 self-center overflow-y-auto overscroll-y-auto rounded-md border border-primary/20 bg-background p-2 py-1 shadow-md scrollbar-thin scrollbar-track-primary/10 scrollbar-thumb-primary/30`}
					>
						<button
							type="button"
							onClick={() => resetState()}
						...
						{items ? (
							items.map((item, index) => (
								<button
									...
									className={`flex w-full cursor-pointer items-center rounded-sm p-2 px-2 hover:bg-primary/20 ${selectedIds?.includes(item.id) ? "bg-primary/20" : ""}`}
								>
