...
import type { TOpportunity, TOpportunityDTOWithClient } from "@/utils/schemas/opportunity.schema";
import type { TPartner, TPartnerDTO } from "@/utils/schemas/partner.schema";
import type { TProposal, TProposalDTO } from "@/utils/schemas/proposal.schema";
	...
	return (
		<div className="relative flex w-full items-center justify-center bg-gray-50">
			...
			<div className="fixed right-4 top-1/2 z-50 hidden -translate-y-1/2 flex-col gap-3 print:hidden lg:flex">
				<button type="button" className="group relative rounded-full bg-white p-3 shadow-lg transition-all hover:bg-gray-50" onClick={() => generatePDF()}>
					<svg className="h-6 w-6 text-gray-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
						<title>Baixar PDF</title>
						<path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
					</svg>
					<span className="absolute right-full mr-2 whitespace-nowrap rounded bg-gray-800 px-2 py-1 text-sm text-white opacity-0 transition-opacity group-hover:opacity-100">
						Baixar PDF
					</span>
				</button>
				...
				<button
					type="button"
					className="group relative rounded-full bg-white p-3 shadow-lg transition-all hover:bg-gray-50"
				...
				<button type="button" className="group relative rounded-full bg-white p-3 shadow-lg transition-all hover:bg-gray-50" onClick={() => window.print()}>
					<svg className="h-6 w-6 text-gray-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
