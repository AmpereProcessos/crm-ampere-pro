...
const PayloadFilters = z.object({
  methodologyIds: z.array(
    z.string({ required_error: 'ID de metodologia de pagamento não informado.', invalid_type_error: 'Tipo não válido para o ID do metodologia de pagamento.' })
  ),
...
const getPaymentMethods: NextApiHandler<TPostResponse> = async (req, res) => {
  ...
  const { methodologyIds, kitsIds, plansIds, productsIds, servicesIds } = PayloadFilters.parse(req.body)
  ...
  if (methodologyIds.length > 0) {
    const paymentMethods = await paymentMethodsCollection.find({ _id: { $in: methodologyIds.map((p) => new ObjectId(p)) } }).toArray()
