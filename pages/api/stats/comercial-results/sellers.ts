...
import { _id } from '@/utils/integrations/auth/personalized-mongodb-adapter'
import { TClient } from '@/utils/schemas/client.schema'
import { TOpportunity } from '@/utils/schemas/opportunity.schema'
import { TProposal } from '@/utils/schemas/proposal.schema'
import { TSaleGoal } from '@/utils/schemas/sale-goal.schema'
import { QueryDatesSchema, QueryPeriodsComparisonSchema } from '@/utils/schemas/stats.schema'
...
const getSalePromotersResultsRoute: NextApiHandler<{ data: TSalePromotersResults }> = async (req, res) => {
...
const getSalePromoterResultByIdRoute: NextApiHandler<{ data: TSalePromoterResultsById }> = async (req, res) => {
  // const session = await validateAuthorization(req, res, 'resultados', 'visualizarComercial', true)
  ...
  const firstPeriodAfterDate = dayjs(firstPeriodAfter).startOf('day').subtract(3, 'hour').toDate()
  ...
  const secondPeriodAfterDate = dayjs(secondPeriodAfter).startOf('day').subtract(3, 'hour').toDate()
...
export default apiHandler({ GET: getSalePromoterResultByIdRoute, POST: getSalePromotersResultsRoute })
