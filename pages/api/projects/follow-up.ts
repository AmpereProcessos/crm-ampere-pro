    ...
    .map((project) => {
      ...
      if (ProjectTypeProcesses.some((p) => HomologationProcessesIds.includes(p))) {
        ...
        if (ProjectTypeProcesses.includes('homologation_initiation')) {
          ...
          if (isHomologationInitiated)
            acc['INICIAÇÃO DE PROJETO'] = { concluido: true, data: homologationElaborationStartDate?.toISOString() || '-' };
        }
        ...
        if (ProjectTypeProcesses.includes('homologation_access_approval')) {
          ...
          if (isHomologationApproved)
            acc['APROVAÇÃO DA CONCESSIONÁRIA'] = { concluido: true, data: homologationAccessResponseDate?.toISOString() || '-' };
        }
      }
      ...
      if (ProjectTypeProcesses.some((p) => ExecutionProcessesIds.includes(p))) {
        ...
        if (ProjectTypeProcesses.includes('execution_planning_post_contract')) {
          ...
          if (isExecutionStarted)
            acc['PLANEJAMENTO PÓS CONTRATO PARA EXECUÇÃO'] = { concluido: true, data: executionStartDate?.toISOString() || '-' };
        }
