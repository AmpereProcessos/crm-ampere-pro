...
const NewLeadQueryInputSchema = z.object({
  ...
  email: z
    ...
    .nullable(),
  uf: z
    .string({ invalid_type_error: "UF deve ser uma string" })
    .min(2, "UF deve ter pelo menos 2 caracteres")
    .optional()
    .nullable(),
  cidade: z
    .string({ invalid_type_error: "Cidade deve ser uma string" })
    .min(3, "Cidade deve ter pelo menos 3 caracteres")
    .optional()
    .nullable(),
  canalAquisicao: z
    .string({ invalid_type_error: "Canal de aquisição deve ser uma string" })
    .optional()
    .nullable(),
  valorFaturaEnergia: z
    .string({ invalid_type_error: "Valor da fatura de energia deve ser um número" })
    .optional()
    .nullable(),
  preferenciaMeioContato: z.enum(["EMAIL", "WHATSAPP", "TELEFONE"]).optional().nullable(),
  // others
  expectativaFechamento: z
...
async function handleLeadGeneration(newLead: z.infer<typeof NewLeadQueryInputSchema>) {
  ...
  return {
    data: { insertedId: insertedOpportunityId, whatsappRedirectMessage },
    message: "Lead gerado com sucesso",
  };
}
...
async function getNewLeadReceiver({
  ...
  try {
    ...
    const lastReceiverIndex =
      leadReceivers.findIndex((r) => r._id.toString() === lastReceiverId) || 0;
    // Increment the index and if it's greater than the length of the list, reset to 0
    const newReceiverIndex =
      lastReceiverIndex + 1 === leadReceivers.length ? 0 : lastReceiverIndex + 1;
