...
const createRequest: NextApiHandler<PostResponse> = async (req, res) => {
  ...
  if (!opportunityId || typeof opportunityId !== "string" || !ObjectId.isValid(opportunityId))
    throw new createHttpError.BadRequest(
      "Referência da oportunidade não encontrada ou inválida, não é possível prosseguir com a solicitação.",
    );
  if (!proposalId || typeof proposalId !== "string" || !ObjectId.isValid(proposalId))
    throw new createHttpError.BadRequest(
      "Referência da proposta não encontrada ou inválida, não é possível prosseguir com a solicitação.",
    );

  const appAmpereDb = await connectToRequestsDatabase();
  const contractRequestsCollection: Collection<TContractRequest> =
    appAmpereDb.collection("contrato");
  ...
  const crmOpportunity = await crmOpportunitiesCollection.findOne({
  ...
  const partner = await partnersCollection.findOne({
    _id: new ObjectId(crmOpportunity.idParceiro),
  });
  ...
  if (!insertContractRequestResponse.acknowledged)
    throw new createHttpError.BadRequest(
      "Oops, houve um erro desconhecido ao solicitar o contrato",
    );
  ...
  await clientsCollection.updateOne(
    { _id: new ObjectId(crmOpportunity.idCliente) },
    { $set: clientUpdates },
  );
  ...
  return res
    .status(201)
    .json({ data: { insertedId }, message: "Solicitação de contrato criada com sucesso !" });
};
