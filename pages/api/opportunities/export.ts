...
const getOpportunitiesExport: NextApiHandler<GetResponse> = async (req, res) => {
  ...
  if (isResponsibleDefined && userScope && !userScope.includes(responsibles))
  ...
  const result = await opportunitiesCollection.aggregate([{ $match: query }, { $project: projection }]).toArray();
  ...
  const exportation = result.map((project) => {
    const info = {
      ...
      ganho: project.ganho,
      valorProposta: project.proposta ? project.proposta.valor : 0,
      potenciaPicoProposta: project.proposta ? project.proposta.potenciaPico : 0,
