...
const createHomologation: NextApiHandler<PostResponse> = async (req, res) => {
	...
	const insertResponse = await insertHomologation({
		collection,
		info: homologation,
		partnerId: partnerId || "",
	});
...
const editHomologation: NextApiHandler<PutResponse> = async (req, res) => {
	...
	const updateResponse = await updateHomologation({
		id: id,
		collection: collection,
		changes: changes,
		query: partnerQuery,
...
export default apiHandler({
	GET: getHomologations,
	POST: createHomologation,
	PUT: editHomologation,
});
