...
import { AppProjectResultsSimplifiedProjection, type TAppProject } from '@/utils/schemas//projects.schema';
...
const QuerySchema = z.object({
  projectType: z.string({
    required_error: 'Tipo de projeto não fornecido ou inválido.',
    invalid_type_error: 'Tipo de projeto não fornecido ou inválido.',
  }),
  after: z
    .string({
...
async function getProcessTrackingStats(request: NextRequest) {
  ...
  if (!(projectType in ProcessTrackedByProjectType))
    throw new createHttpError.BadRequest('Tipo de projeto selecionado ainda não possui processos rastreados.');
  ...
  const reduced = projects.reduce((acc: TProcessTrackingStats, current) => {
    ...
    if (ProjectTypeProcesses.some((p) => ContractProcessesIds.includes(p))) {
      ...
      if (ProjectTypeProcesses.includes('contract_formulation')) {
        if (!acc.CONTRATOS['FORMULAÇÃO DE CONTRATO'])
        ...
        if (wasFormulatedWithinPeriod) {
          const time =
            contractSolicitationDate && contractLiberationDate
              ? getHoursDiff({ start: contractSolicitationDate, finish: contractLiberationDate }) || 8
              : 0;
      ...
      if (ProjectTypeProcesses.includes('contract_signature_collection')) {
        ...
        if (wasSignedWithinPeriod) {
          const time =
            contractLiberationDate && contractSignatureDate ? getHoursDiff({ start: contractLiberationDate, finish: contractSignatureDate }) || 8 : 0;
    ...
    if (ProjectTypeProcesses.some((p) => HomologationProcessesIds.includes(p))) {
      ...
      if (ProjectTypeProcesses.includes('homologation_initiation')) {
        if (!acc.HOMOLOGAÇÃO['INICIAÇÃO DE PROJETO'])
          acc.HOMOLOGAÇÃO['INICIAÇÃO DE PROJETO'] = { andamento: 0, concluidos: 0, tempoTotalConclusao: 0 };
      ...
      if (ProjectTypeProcesses.includes('homologation_inspection_request')) {
        ...
        if (wasInspectionRequestedWithinPeriod) {
          const time =
            executionFinishDate && inspectionRequestDate ? getHoursDiff({ start: executionFinishDate, finish: inspectionRequestDate }) || 8 : 0;
      ...
      if (ProjectTypeProcesses.includes('homologation_inspection_effectuation')) {
        if (!acc.HOMOLOGAÇÃO['APROVAÇÃO DA VISTORIA'])
          acc.HOMOLOGAÇÃO['APROVAÇÃO DA VISTORIA'] = { andamento: 0, concluidos: 0, tempoTotalConclusao: 0 };
        const inspectionRequestDate = getDateFromString(current.homologacao.dataSolicitacaoVistoria);
        ...
        const wasInspectionEffectuatedWithinPeriod = getDateIsWithinPeriod({
          date: inspectionEffectuationDate,
          after: afterDate,
          before: beforeDate,
        });
        if (wasInspectionEffectuatedWithinPeriod) {
          const time =
            inspectionRequestDate && inspectionEffectuationDate
              ? getHoursDiff({ start: inspectionRequestDate, finish: inspectionEffectuationDate }) || 8
              : 0;
    ...
    if (ProjectTypeProcesses.some((p) => SupplyProcessesIds.includes(p))) {
      ...
      if (ProjectTypeProcesses.includes('supply_purchase_release')) {
        if (!acc.SUPRIMENTOS['LIBERAÇÃO PARA COMPRA'])
          acc.SUPRIMENTOS['LIBERAÇÃO PARA COMPRA'] = { andamento: 0, concluidos: 0, tempoTotalConclusao: 0 };
        ...
        if (wasPurchaseReleasedWithinPeriod) {
          const time =
            contractSignatureDate && supplyLiberationDate ? getHoursDiff({ start: contractSignatureDate, finish: supplyLiberationDate }) || 8 : 0;
    ...
    if (ProjectTypeProcesses.some((p) => ExecutionProcessesIds.includes(p))) {
      ...
      if (ProjectTypeProcesses.includes('execution_post_contract_planning')) {
        ...
        if (wasPostContractPlannedWithinPeriod) {
          const time =
            contractSignatureDate && executionStartDate ? getHoursDiff({ start: contractSignatureDate, finish: executionStartDate }) || 8 : 0;
    ...
    const projects: TSimplifiedProjectResult[] = result.map((project) => {
      ...
      return {
        ...
        valor: getContractValue({
          system: info.sistema.valorProjeto || 0,
          pa: info.padrao.valor || 0,
          structure: info.estruturaPersonalizada.valor || 0,
        }),
        contrato: {
