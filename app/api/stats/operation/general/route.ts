...
import type { TAppProject } from '@/utils/schemas//projects.schema';
...
async function getOperationGeneralStats(request: NextRequest) {
  ...
  return NextResponse.json({ data: { vendas: sales, nps, tiposProjeto: projectsByType } });
}

export const GET = apiHandler({
  GET: getOperationGeneralStats,
});
...
async function getContractsWithinPeriod({ afterDate, query, beforeDate, collection }: GetContractsWithinPeriodParams): Promise<TSalesValuesGraph> {
  ...
  const datesStrs = getDayStringsBetweenDates({
    initialDate: dayjs(afterDateStr).add(3, 'hours').toISOString(),
...
export type TGeneralOperationStatsRouteOutput = UnwrapNextResponse<Awaited<ReturnType<typeof getOperationGeneralStats>>>;
