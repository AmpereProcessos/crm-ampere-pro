...
export type TGetCreditorsResultInput = z.infer<
	typeof GetCreditorsResultInputSchema
>;
async function getCreditorsResults({
	input,
	session,
}: {
	input: TGetCreditorsResultInput;
	...
	const crmProjectTypesCollection =
		crmDb.collection<TProjectType>("project-types");
	const appDb = await connectToAmpereProjectsDatabase();
	const appProjectsCollection = appDb.collection<TProject>("dados");
	const crmUsers = await crmUsersCollection
		.find({}, { projection: { _id: 1, nome: 1 } })
		.toArray();
	const crmProjectTypes = await crmProjectTypesCollection
		.find({}, { projection: { _id: 1, nome: 1 } })
		.toArray();

	const responsiblesAsSellers = (responsibles
		?.map((r) => crmUsers.find((u) => u._id.toString() === r)?._id.toString())
		.filter((r) => !!r) ?? []) as string[];

	const projectTypesIdsAsProjectTypes = (projectTypes
		?.map((p) =>
			crmProjectTypes.find((t) => t._id.toString() === p)?._id.toString(),
		)
		.filter((r) => !!r) ?? []) as string[];
	...
	const responsiblesQuery: Filter<TProject> =
		responsiblesAsSellers.length > 0
			? { "vendedor.nome": { $in: responsiblesAsSellers } }
			: {};
	const projectTypesQuery: Filter<TProject> =
		projectTypesIdsAsProjectTypes.length > 0
			? { tipoDeServico: { $in: projectTypesIdsAsProjectTypes } }
			: {};
	...
	console.log(
		"[INFO] [GET_CREDITORS_RESULTS] Overall Sales Result",
		overallSalesResult,
	);
	...
	const byCreditorTotalQty = byCreditorResult.reduce(
		(acc, curr) => acc + curr.qtdeVendida,
		0,
	);
	const byCreditorTotalValue = byCreditorResult.reduce(
		(acc, curr) =>
			acc +
			curr.valorProjetoVendido +
			curr.valorOeMVendido +
			curr.valorPadraoVendido +
			curr.valorEstruturaPersonalizadaVendido +
			curr.valorSeguroVendido,
		0,
	);
	...
	const byCreditor = Object.entries(byCreditorReduced).map(
		([creditor, data]) => ({
			creditor,
...
export type TGetCreditorsResultOutput = Awaited<
	ReturnType<typeof getCreditorsResults>
>;
