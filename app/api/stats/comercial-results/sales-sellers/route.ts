...
async function getSalesTeamResults(request: NextRequest) {
  ...
  const salesTeamResults = projects.reduce((acc: TSellerSalesResults, current) => {
    ...
    if (cameFromInsideSales) {
      ...
      if (signingPeriod) {
        if (!acc[seller.nome].potenciaPico.origem.externo[clientAquisitionOrigin])
          acc[seller.nome].potenciaPico.origem.externo[clientAquisitionOrigin] = 0;

        acc[seller.nome].potenciaPico.origem.externo[clientAquisitionOrigin] += proposePeakPower;
      }
      if (signingPeriod) {
        if (!acc[seller.nome].valorVendido.origem.externo[clientAquisitionOrigin])
          acc[seller.nome].valorVendido.origem.externo[clientAquisitionOrigin] = 0;
