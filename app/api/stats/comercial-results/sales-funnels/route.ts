...
async function getInProgressResults(request: NextRequest) {
  ...
  console.log('[INFO] [GET_SALES_FUNNELS_DATA] Query Params', searchParamsObject);
  const payload = await request.json();
  console.log('[INFO] [GET_SALES_FUNNELS_DATA] Payload', payload);
  ...
  const inProgressResults = projects.reduce((acc, current) => {
    ...
    currentProjectFunnels?.forEach((funnel, index: number) => {
      ...
      if (wasLostInCurrentPeriod) {
        ...
        if (!acc[funnelName][stageName].perdas.perdasPorMotivo[lossReason || ''])
          acc[funnelName][stageName].perdas.perdasPorMotivo[lossReason || ''] = 0;
...
async function getOpportunities({
  ...
  try {
    ...
    const match = {
      ...
      ...projectTypesQuery,
      $or: [
        { 'ganho.data': null, 'perda.data': null },
