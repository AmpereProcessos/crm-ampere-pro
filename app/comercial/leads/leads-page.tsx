...
function LeadsPageFilters({ filters, updateFilters }: LeadsPageFiltersProps) {
	...
	const periodFieldOptions: {
		id: number;
		label: string;
		value: TGetManyLeadsInput["periodField"];
	}[] = [
			...
			<div className="flex w-full flex-col items-center justify-start gap-2 md:flex-row flex-wrap">
				...
				<div className="w-full md:w-[300px]">
					<SelectInput
						...
						handleChange={(value) =>
							updateFilters({
								periodField: value as TGetManyLeadsInput["periodField"],
							})
						}
				...
				<div className="w-full md:w-[300px]">
					<MultipleSelectWithImages
						...
						handleChange={(value) =>
							updateFilters({
								qualifiersIds: value as TGetManyLeadsInput["qualifiersIds"],
							})
						}
						options={
...
function LeadCard({
	...
	const { mutate: handleUpdateLeadMutation, isPending } = useMutation({
		...
		mutationFn: async () =>
			await updateLead({
				id: lead._id,
				lead: { dataUltimoContato: new Date().toISOString() },
			}),
		onMutate: () => {
