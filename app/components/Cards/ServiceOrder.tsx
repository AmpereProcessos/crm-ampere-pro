...
function getUrgencyTag(urgency: TServiceOrderDTO["urgencia"]) {
	...
	if (!urgency)
		return (
			<div className="flex items-center gap-1 rounded-lg bg-gray-600 px-2 py-0.5 text-white">
				<TbUrgent size={16} />
		...
		<div className="flex w-full flex-col gap-2 rounded-md border border-gray-500 bg-[#fff] p-2">
			<div className="flex w-full flex-col-reverse items-center justify-between gap-2 lg:flex-row">
				<div className="flex w-full flex-wrap items-center justify-start gap-1 lg:w-fit">
					...
					{true ? (
						<h1 onClick={() => handleClick(serviceOrder._id)} className="cursor-pointer text-sm font-black leading-none tracking-tight duration-300 ease-in-out hover:text-cyan-500">
			...
			<div className="flex w-full flex-col items-center justify-between gap-2 lg:flex-row">
				<div className="flex grow flex-col items-center gap-2 lg:flex-row">
					<div className="flex flex-wrap items-center gap-1">
						...
						<p className="text-[0.6rem] font-medium leading-none tracking-tight">{formatLocation({ location: serviceOrder.localizacao, includeCity: true, includeUf: true })}</p>
					</div>
					<div className="flex flex-wrap items-center gap-1">
						<FaUser size={12} />
						...
						{serviceOrder.responsaveis.map((responsible, index) => (
							<div key={index} className="flex items-center gap-2 rounded border border-gray-500 bg-[#f8f8f8] px-2 py-0.5 shadow-md">
				...
				<div className="flex items-center gap-2">
					...
					<div className="flex items-center gap-1">
						...
						<p className="text-[0.65rem] font-medium text-gray-500">{serviceOrder.autor.nome}</p>
					</div>
				</div>
			</div>
		</div>
