...
import { FaPhone, FaRegFolder, FaUser } from "react-icons/fa";
import { FaDiamond, FaLocationDot } from "react-icons/fa6";
import { MdMenu } from "react-icons/md";
import { TbUrgent } from "react-icons/tb";
import ReportPageSession from "./ReportPageSession";
import Avatar from "@/components/utils/Avatar";
import { VscDebugBreakpointDataUnverified, VscDebugBreakpointLog } from "react-icons/vsc";
import { renderCategoryIcon } from "@/lib/methods/rendering";
import { TFileReference, TFileReferenceDTO } from "@/utils/schemas/file-reference.schema";
...
import { editServiceOrder, editServiceOrderPersonalized } from "@/utils/mutations/service-orders";
...
function ReportPage({ user, order }: ReportPageProps) {
	...
	async function concludedReportSession({
		...
		try {
			...
			await editServiceOrderPersonalized({ id: order._id, changes: { [`relatorio.secoes.${currentSessionIndex}.dataConclusao`]: new Date().toISOString() } });
	...
	if (finishReportSuccess)
		return (
			<div className="flex w-full grow flex-col items-center justify-center gap-2 text-green-500">
				...
				<p className="text-lg font-medium tracking-tight text-gray-500">RELATÓRIO FINALIZADO COM SUCESSO !</p>
			</div>
		);
	return (
		<div className="flex h-full w-full flex-col gap-6">
			<h1 className="w-full bg-black px-8 py-4 text-center text-base font-black tracking-tight text-white lg:text-xl">RELATÓRIO DE CONCLUSÃO DE ORDEM DE SERVIÇO</h1>
			<div className="flex w-full flex-col gap-3 px-6 py-2">
				<div className="flex w-full flex-col gap-3 lg:flex-row">
					<div className="flex w-full flex-col gap-3 p-3 lg:w-1/2">
						...
						<div className="flex w-full flex-col items-center justify-start gap-1">
							...
							<div className="flex w-full flex-col items-start justify-start gap-2 lg:flex-row lg:items-center lg:gap-6">
								...
								<div className="flex w-fit items-center gap-1">
									<FaLocationDot />
									<h1 className="text-[0.85rem] font-[600] tracking-tight">{formatLocation({ location: order.localizacao, includeCity: true, includeUf: true })}</h1>
								</div>
							</div>
						</div>
					</div>
					<div className="flex w-full flex-col gap-3 p-3 lg:w-1/2">
						...
						<div className="flex w-full flex-col items-center justify-center gap-1">
							<h1 className="w-full text-[0.85rem] font-medium tracking-tight text-gray-500 lg:text-start">ESSAS SÃO AS OBSERVAÇÕES DEFINIDAS PARA EXECUÇÃO DESSA ORDEM DE SERVIÇO:</h1>
				...
				<div className="flex w-full flex-col gap-3 p-3">
					...
					<div className="flex w-full flex-col gap-3 lg:flex-row">
						<div className="flex w-full flex-col gap-1 rounded border border-gray-800 lg:w-1/2">
							<h1 className="w-full bg-gray-800 px-2 py-1 text-center text-sm font-bold text-white">MATERIAIS DISPONÍVEIS IN LOCO</h1>
							<div className="flex w-full flex-col gap-1 p-2">
								{order.materiais.disponiveis.length > 0 ? (
									order.materiais.disponiveis.map((material, index) => (
										<div key={index} className="flex items-center gap-2">
											<div className="flex h-[15] w-[15] items-center justify-center rounded-full border border-black p-1 text-[15px]">{renderCategoryIcon(material.categoria)}</div>
						...
						<div className="flex w-full flex-col gap-1 rounded border border-gray-800 lg:w-1/2">
							<h1 className="w-full bg-gray-800 px-2 py-1 text-center text-sm font-bold text-white">MATERIAIS PARA RETIRADA</h1>
							<div className="flex w-full flex-col gap-1 p-2">
			...
			<div className="flex w-full items-center justify-center p-2">
				{isLastSession ? (
					<button
						...
						className="h-9 w-full whitespace-nowrap rounded bg-green-700 px-4 py-2 text-sm font-medium text-white shadow disabled:bg-gray-500 disabled:text-white enabled:hover:bg-green-600 enabled:hover:text-white lg:w-[350px]"
					>
						FINALIZAR RELATÓRIO
					</button>
				) : (
					<button
						...
						className="h-9 w-full whitespace-nowrap rounded bg-blue-700 px-4 py-2 text-sm font-medium text-white shadow disabled:bg-gray-500 disabled:text-white enabled:hover:bg-blue-600 enabled:hover:text-white lg:w-[350px]"
					>
						PRÓXIMA ETAPA
					</button>
				)}
