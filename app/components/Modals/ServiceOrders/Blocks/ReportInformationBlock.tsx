...
function ReportInformationBlock({ orderId, infoHolder, setInfoHolder }: ReportInformationBlockProps) {
  ...
  return (
    <div className="flex w-full flex-col gap-y-2">
      <h1 className="w-full rounded bg-gray-700 p-1 text-center font-bold text-white">RELATÓRIO DE CONCLUSÃO</h1>
      ...
      {infoHolder.relatorio.aplicavel && infoHolder.relatorio.secoes.length == 0 ? (
        <div className="flex w-full flex-col items-center justify-center gap-1">
          ...
          <button onClick={() => useReportModel(infoHolder)} className="self-cente w-fit bg-blue-800 px-2 py-1 text-sm font-bold tracking-tight text-white">
            UTILIZAR MODELO
          </button>
        </div>
      ) : null}
