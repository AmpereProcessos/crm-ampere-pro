...
function ObservationsBlock({ infoHolder, setInfoHolder }: ObservationsBlockProps) {
  ...
  return (
    <div className='flex w-full flex-col gap-y-2'>
      <h1 className='w-full bg-primary/50 p-1 text-center text-xs font-medium text-white'>OBSERVAÇÕES P/ EXECUÇÃO</h1>
          ...
          <div className='flex w-full flex-col gap-1'>
            ...
            {infoHolder.observacoes.map((observation, index) => (
              <div key={index} className='flex w-full flex-col rounded-md border border-primary/50'>
                ...
                <h1 className='w-full p-2 text-center text-xs font-medium tracking-tight text-primary/50'>{observation.descricao}</h1>
              </div>
            ))}
          </div>
        ) : (
