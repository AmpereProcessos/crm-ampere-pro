	...
	return (
		<div className="flex w-full flex-col gap-2">
			...
			<div className="flex items-center justify-end">
				<button className="rounded bg-black p-1 px-4 text-sm font-medium text-white duration-300 ease-in-out hover:bg-gray-700" onClick={() => addUpdate(updateHolder)}>
				...
				{infoHolder.atualizacoes.length > 0 ? (
					infoHolder.atualizacoes.map((update, index) => (
						<div key={index} className="flex w-full flex-col gap-1 rounded-md border border-gray-500 p-3">
							...
							<div className="flex w-full items-center justify-center rounded-md bg-gray-100 p-2 text-center text-xs font-medium text-gray-500">{update.descricao}</div>
						</div>
					))
				) : (
					<p className="flex w-full grow items-center justify-center py-2 text-center font-medium italic tracking-tight text-gray-500">Sem atualizações adicionadas.</p>
				)}
			</div>
		</div>
	);
