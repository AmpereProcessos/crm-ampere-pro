...
function rankAndLimit(
  items: TGlobalSearchResultItem[],
  search: string,
  limit: number,
): TGlobalSearchResultItem[] {
  ...
  return items
    .map((item, index) => {
      ...
      let score = 4;
      if (label === normalizedSearch || (digits.length >= 3 && searchableDigits === digits))
        score = 0;
...
function includesEntity(
  activeEntities: TGlobalSearchEntity[],
...
export async function globalSearch(
  ...
  input: TGlobalSearchInput,
): Promise<TGlobalSearchResults> {
  const { q, limit } = input;
  ...
  const clientVisibility = buildResourceVisibilityFilter(
    session,
    "clientes",
  ) as Filter<TClient> | null;
  console.log("[GLOBAL_SEARCH] Client visibility", clientVisibility);
  if (clientVisibility && includesEntity(activeEntities, "clients")) {
    ...
    const searchConditions: Filter<TClient>[] = [
      ...
      ...(digitsRegex
        ? [
            { cpfCnpj: digitsRegex },
            { telefonePrimario: digitsRegex },
            { telefonePrimarioBase: digitsRegex },
          ]
        : []),
    ];
    tasks.push(
        ...
        .then((rows) => {
          results.clients = rankAndLimit(
            rows.map((row) => ({
              ...
              label: row.nome,
              description:
                [row.cpfCnpj, row.telefonePrimario, row.email].filter(Boolean).join(" · ") || null,
  ...
  const opportunityVisibility = buildResourceVisibilityFilter(
    session,
    "oportunidades",
  ) as Filter<TOpportunity> | null;
  console.log("[GLOBAL_SEARCH] Opportunity visibility", opportunityVisibility);
  if (opportunityVisibility && includesEntity(activeEntities, "opportunities")) {
    ...
    const searchConditions: Filter<TOpportunity>[] = [
      ...
      ...(digitsRegex
        ? [{ "cliente.cpfCnpj": digitsRegex }, { "cliente.telefonePrimario": digitsRegex }]
        : []),
    ];
    tasks.push(
        ...
        .then((rows) => {
          results.opportunities = rankAndLimit(
            rows.map((row) => ({
              ...
              label: row.nome,
              description:
                [`#${row.identificador}`, row.cliente?.nome].filter(Boolean).join(" · ") || null,
  ...
  if (proposalVisibility && includesEntity(activeEntities, "proposals")) {
    ...
    const searchConditions: Filter<TProposal>[] = [
      { nome: textRegex },
    ...
    tasks.push(
      proposalsCollection
        .find(
          { ...proposalVisibility, $or: searchConditions },
          { projection: { nome: 1, valor: 1, oportunidade: 1 } },
        )
        .sort({ _id: -1 })
        .limit(fetchLimit)
        .toArray()
        .then((rows) => {
          results.proposals = rankAndLimit(
            rows.map((row) => ({
              ...
              label: row.nome,
              description:
                [row.valor ? moneyFormatter.format(row.valor) : null, row.oportunidade?.nome]
                  .filter(Boolean)
                  .join(" · ") || null,
  ...
  const analysisVisibility = buildResourceVisibilityFilter(
    session,
    "analisesTecnicas",
  ) as Filter<TTechnicalAnalysis> | null;
  console.log("[GLOBAL_SEARCH] Technical Analysis visibility", analysisVisibility);
  ...
  if (analysisVisibility && includesEntity(activeEntities, "technicalAnalysis")) {
    ...
    tasks.push(
      collection
        .find(
          { ...analysisVisibility, $or: conditions },
          { projection: { nome: 1, status: 1, oportunidade: 1 } },
        )
        .sort({ _id: -1 })
        .limit(fetchLimit)
        .toArray()
  ...
  const homologationVisibility = buildResourceVisibilityFilter(
    session,
    "homologacoes",
  ) as Filter<THomologation> | null;
  if (homologationVisibility && includesEntity(activeEntities, "homologations")) {
    ...
    const conditions: Filter<THomologation>[] = [
      ...
      { "oportunidade.nome": textRegex },
      ...(digitsRegex
        ? [{ "titular.identificador": digitsRegex }, { "titular.contato": digitsRegex }]
        : []),
    ];
    tasks.push(
      collection
  ...
  if (canViewResource(session, "projetos") && includesEntity(activeEntities, "projects")) {
    tasks.push(
      (async () => {
        ...
        const visibilityConditions: Filter<TProject>[] = partnerIds
        ...
        if (responsibleScope !== null && responsibleScope !== undefined) {
          ...
          const validResponsibleIds = responsibleScope
            .filter((id) => ObjectId.isValid(id))
            .map((id) => new ObjectId(id));
          const scopedUsers = await usersCollection
            .find({ _id: { $in: validResponsibleIds } }, { projection: { nome: 1 } })
            .toArray();
        ...
        const conditions: Filter<TProject>[] = [
          ...
          ...(digitsRegex
            ? [{ cpf_cnpj: digitsRegex }, { telefone: digitsRegex }, { codigoSVB: digitsRegex }]
            : []),
        ];
        ...
        results.projects = rankAndLimit(
          rows.map((row) => ({
            ...
            description:
              [
                row.qtde ? `#${row.qtde}` : row.codigoSVB ? `SVB ${row.codigoSVB}` : null,
