...
export async function getOpportunitiesByQuery({ collection, query }: GetOpportunityByQueryParams) {
	try {
		...
		const opportunities = (await collection
			.aggregate([{ $match: { ...query, dataExclusao: null } }, { $addFields: addFields }, { $lookup: lookup }, { $project: projection }, { $sort: sort }])
...
export async function getOpportunitiesSimplified({ collection, query }: GetOpportunitiesSimplifiedParams) {
	try {
		const projects = await collection.find({ ...query, dataExclusao: null }, { projection: SimplifiedOpportunityWithProposalProjection, sort: { _id: -1 } }).toArray();
