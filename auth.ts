...
// Define the refresh interval (5 minutes in milliseconds)
...
export const { auth, handlers, signIn, signOut, unstable_update } = NextAuth({
	...
	callbacks: {
		...
		async jwt({ token, user, account, profile, isNewUser }) {
			// Check if we need to refresh the token data
			const now = Date.now();
			...
			if (now - lastUpdated > REFRESH_INTERVAL) {
				...
				try {
					...
					if (userInDb) {
						const userPartner = await partnersCollection.findOne({
							_id: new ObjectId(userInDb.idParceiro),
						});
